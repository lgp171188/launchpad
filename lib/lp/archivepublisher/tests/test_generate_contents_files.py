...
class TestGenerateContentsFiles(TestCaseWithFactory):
    ...
    def test_name_is_unique_for_each_distro(self):
        ...
        self.assertNotEqual(
            GenerateContentsFiles(
                test_args=["-d", self.factory.makeDistribution().name]
            ).name,
            GenerateContentsFiles(
                test_args=["-d", self.factory.makeDistribution().name]
            ).name,
    ...
    def test_writeAptContentsConf_writes_header(self):
        ...
        with open(
            ...
            % (script.content_archive, distro.name)
        ) as f:
    ...
    def test_main(self):
        ...
        self.factory.makeBinaryPackageBuild(
            distroarchseries=das, pocket=package.pocket, processor=processor
        )
        suite = package.suite
        script = self.makeScript(distro)
