...
class TestExportTranslationsToBranch(TestCaseWithFactory):
    ...
    def test_translations_export_to_branch(self):
        ...
        self.assertIn(
            "Processed 1 item(s); 0 failure(s), 0 unpushed branch(es).", stderr
        )
        self.assertEqual(
    ...
    def test_handleUnpushedBranch_mails_branch_owner(self):
        ...
        (sender, recipients, subject, text), kwargs = exporter._sendMail.calls[
            -1
        ]
    ...
    def test_findChangedPOFiles_unchanged(self):
        ...
        self.assertEqual(
            ...
            list(
                exporter._findChangedPOFiles(
                    pofile.potemplate.productseries, date_in_the_future
                )
            ),
        )
