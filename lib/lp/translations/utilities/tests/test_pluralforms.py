...
class PluralFormsTest(unittest.TestCase):
    ...
    def test_make_friendly_plural_forms(self):
        ...
        self.assertEqual(
            single_form, [{"examples": [0, 1, 2, 3, 4, 5], "form": 0}]
        )
    ...
    def test_make_friendly_plural_form_failures(self):
        ...
        self.assertRaises(
            BadPluralExpression, make_friendly_plural_forms, "n**2", 1
        )

        # Expressions longer than 500 characters are not accepted.
        long_but_valid = "+".join(["0"] * 500)
        self.assertRaises(
            BadPluralExpression, make_friendly_plural_forms, long_but_valid, 1
        )

        # Only "n" is allowed as a variable.
        self.assertRaises(
            BadPluralExpression, make_friendly_plural_forms, "(a=1)", 1
        )

        # Dividing by zero doesn't work.
        self.assertRaises(
            BadPluralExpression, make_friendly_plural_forms, "(n/0)", 1
        )

        # The modulo operator does not allow divide-by-zero either.
        self.assertRaises(
            BadPluralExpression, make_friendly_plural_forms, "3%n", 1
        )

        # Must discover the expected number of forms.
        self.assertRaises(
            BadPluralExpression, make_friendly_plural_forms, "n!=1", 3
        )
        ...
        self.assertRaises(
            BadPluralExpression, make_friendly_plural_forms, "n==0 ? 1 : 2", 2
        )
    ...
    def test_make_friendly_plural_form_zero_handling(self):
        ...
        self.assertRaises(
            BadPluralExpression, make_friendly_plural_forms, "(1/n)", 1
        )
