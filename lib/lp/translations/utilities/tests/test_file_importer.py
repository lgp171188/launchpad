...
TEST_TRANSLATION_FILE_WITH_ERROR = (
    r"""
msgid ""
msgstr ""
...
class FileImporterTestCase(TestCaseWithFactory):
    ...
    def _createFileImporters(self, pot_content, po_content, by_maintainer):
        ...
        pot_importer = self._createPOTFileImporter(pot_content, by_maintainer)
    ...
    def test_FileImporter_importMessage_NotImplemented(self):
        ...
        self.assertRaises(NotImplementedError, importer.importMessage, None)

    def test_FileImporter_format_exporter(self):
        # Test if format_exporter behaves like a singleton
        importer = self._createFileImporter()
        self.assertIsNone(
            importer._cached_format_exporter,
    ...
    def test_FileImporter_getOrCreatePOTMsgSet(self):
        ...
        # the DB
        potmsgset1 = pot_importer.potemplate.getPOTMsgSetByMsgIDText(
    ...
    def _test_storeTranslationsInDatabase_empty(self, by_maintainer=True):
        ...
        translation = importer.storeTranslationsInDatabase(message, potmsgset)
    ...
    def test_FileImporter_storeTranslationsInDatabase_privileges(self):
        ...
        entry = removeSecurityProxy(po_importer.translation_import_queue_entry)
        entry.importer = po_importer.translation_import_queue_entry.importer
        is_editor = po_importer.pofile.canEditTranslations(unprivileged_person)
        self.assertFalse(
            is_editor, "Unprivileged person is a translations editor."
        )
    ...
    def test_FileImporter_init(self):
        ...
        self.assertIsNotNone(
            pot_importer.potemplate,
            "POTFileImporter had no reference to an IPOTemplate.",
        )
        self.assertTrue(
            pot_importer.pofile is None
            or pot_importer.pofile.language == "en",
    ...
    def test_FileImporter_importFile_conflict(self):
        ...
        po_importer2 = self._createPOFileImporter(
            pot_importer,
            TEST_TRANSLATION_EXPORTED_EARLIER,
            False,
            po_importer.pofile,
        )
        ...
        self.assertTrue(
            errors[0]["error-message"].find(
                "updated by someone else after you"
            )
            != -1,
...
class CreateFileImporterTestCase(TestCaseWithFactory):
    ...
    def _make_queue_entry(self, by_maintainer):
        ...
        pofile.header = (
            "PO-Revision-Date: 2009-01-05 13:22+0000\n"
            "Content-Type: text/plain; charset=UTF-8\n"
        )
...
class FileImporterSharingTest(TestCaseWithFactory):
    ...
    def _makeImportEntry(
        self, side, by_maintainer=False, uploader=None, no_upstream=False
    ):
    ...
    def test_translator_permissions(self):
        ...
        self.assertFalse(pofile.canEditTranslations(self.factory.makePerson()))
        self.assertTrue(pofile.canEditTranslations(self.translator.translator))
