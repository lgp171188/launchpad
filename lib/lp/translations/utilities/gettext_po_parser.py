...
class POHeader:
    ...
    def _parseAssignments(
        self, text, separator=";", assigner="=", skipfirst=False
    ):
    ...
    def _parseHeaderFields(self):
        ...
        for key, value in self._header_dictionary.items():
            if key == "plural-forms":
                ...
                self.translation_revision_date = self._parseOptionalDate(value)
    ...
    def updateFromTemplateHeader(self, template_header):
        ...
        for field in fields_to_copy:
            if field in template_header_dictionary:
                self._header_dictionary[field] = template_header_dictionary[
                    field
                ]
...
class POParser:
    ...
    def _decode(self):
        ...
        try:
            ...
            newchars, length = decode(
                self._pending_chars[: exc.start], "strict"
            )
    ...
    def parse(self, content_bytes):
        ...
        if line is None:
            if (
                self._translation_file.header is None
    ...
    def _storeCurrentMessage(self):
        if self._message is not None:
            ...
            if (
                self._message.msgid_plural
                and number_plural_forms is not None
    ...
    def _parseHeader(self, header_text, header_comment):
        try:
            ...
            raise
        self._translation_file.header.is_fuzzy = "fuzzy" in self._message.flags
    ...
    def _unescapeNumericCharSequence(self, string):
        ...
        unescaped_string = (
            escaped_string.encode("ascii")
            .decode("unicode-escape")
            .encode("iso-8859-1")
        )

        if (
            self._translation_file is not None
        ...
        try:
            ...
            if know_charset:
                ...
                message = (
                    "Could not decode escaped string: (%s)" % escaped_string
                )
            raise TranslationFormatInvalidInputError(
        ...
        while len(string) > 0:
            if string[0] == '"':
                ...
                if len(string) == 1:
                    ...
                    unescaped, string = self._unescapeNumericCharSequence(
                        string
                    )
                    output += unescaped
            else:
    ...
    def _parseFreshLine(self, line, original_line):
        ...
        if line.startswith("msgid_plural"):
            ...
            if self._section is not None and (
                self._section == "msgctxt" or self._section.startswith("msgid")
            ):
                raise TranslationFormatSyntaxError(
                    line_number=self._lineno,
