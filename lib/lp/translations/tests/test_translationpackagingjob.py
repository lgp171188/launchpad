...
def make_translation_merge_job(factory, not_ubuntu=False):
    ...
    return TranslationPackagingJob.makeSubclass(
        ...
        .order_by(Desc(TranslationSharingJob.id))
        .first()
    )
...
class JobFinder:
    def __init__(
        self, productseries, sourcepackage, job_class, potemplate=None
    ):
    ...
    def find(self):
        if self.potemplate is None:
            ...
            return list(
                TranslationSharingJobDerived.iterReady(
                    [
                        TranslationSharingJob.potemplate_id
                        == self.potemplate.id,
                        TranslationSharingJob.job_type == self.job_type,
                    ]
                )
...
class TestTranslationMergeJob(TestCaseWithFactory):
    ...
    def test_getNextJobStatus_status(self):
        ...
        job.start()
        self.assertEqual(
            JobStatus.RUNNING, TranslationMergeJob.getNextJobStatus(packaging)
        )
        job.fail()
        self.assertIs(None, TranslationMergeJob.getNextJobStatus(packaging))
...
class TestTranslationTemplateChangeJob(TestCaseWithFactory):
    ...
    def test_splits_and_merges(self):
        ...
        old_shared = self.factory.makePOTemplate(
            name="template", productseries=other_ps
        )
        new_shared = self.factory.makePOTemplate(
            name="renamed", productseries=other_ps
        )
