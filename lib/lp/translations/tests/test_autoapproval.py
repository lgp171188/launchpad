...
class TestCustomLanguageCode(TestCaseWithFactory):
    ...
    def setUp(self):
        ...
        self.product_codes["pt_PT"] = CustomLanguageCode(
            translation_target=self.product,
            language_code="pt_PT",
...
class TestGuessPOFileCustomLanguageCode(
    TestCaseWithFactory, GardenerDbUserMixin
):
...
class TestTemplateGuess(TestCaseWithFactory, GardenerDbUserMixin):
    ...
    def test_ByPathAndOrigin_similar_between_distroseries(self):
        ...
        # distroseries.
        self._setUpDistro()
        other_series = self.factory.makeDistroSeries(distribution=self.distro)
    ...
    def test_ByDomain_may_return_None(self):
        ...
        entry = self.factory.makeTranslationImportQueueEntry()
        self.assertEqual(
            None, removeSecurityProxy(entry).matchPOTemplateByDomain("domain")
        )
    ...
    def test_ByDomain_reports_conflicts(self):
        ...
        with self.beingTheGardener():
            result = removeSecurityProxy(entry).matchPOTemplateByDomain(domain)
...
class TestKdePOFileGuess(TestCaseWithFactory, GardenerDbUserMixin):
    ...
    def setUp(self):
        ...
        ca_template = self.factory.makePOTemplate(
            distroseries=self.distroseries,
            sourcepackagename=kde3_package,
            name="kde3",
        ...
        nl_template = self.factory.makePOTemplate(
            distroseries=self.distroseries,
            sourcepackagename=kde4_package,
            name="kde4",
...
class TestCleanup(TestCaseWithFactory, GardenerDbUserMixin):
    ...
    def _exists(self, entry_id):
        ...
        entry = self.store.find(
            TranslationImportQueueEntry,
            TranslationImportQueueEntry.id == entry_id,
        ).any()
    ...
    def test_cleanUpObsoleteEntries_unaffected_statuses(self):
        ...
        # they are.
        unaffected_statuses = set(RosettaImportStatus.items) - set(
            translation_import_queue_entry_age.keys()
        )
        self.assertNotEqual(
            0,
            len(unaffected_statuses),
...
class TestAutoBlocking(TestCaseWithFactory):
    ...
    def _makeTemplateEntry(
        self, suffix=".pot", directory=None, status=None, same_target_as=None
    ):
    ...
    def test_getBlockableDirectories_ignores_translations(self):
        ...
        self._makeTranslationEntry("gl.po", status=RosettaImportStatus.BLOCKED)
