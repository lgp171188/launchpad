...
class TestCanonicalUrlForCraftRecipeBuild(TestCaseWithFactory):
    ...
    def test_canonical_url(self):
        ...
        self.assertThat(
            canonical_url(build),
            StartsWith(
                "http://launchpad.test/~person/craft-project/+craft/craft/"
                "+build/"
            ),
        )


class TestCraftRecipeBuildView(TestCaseWithFactory):
    ...
    def test_files(self):
        # CraftRecipeBuildView.files returns all the associated files.
        build = self.factory.makeCraftRecipeBuild(
        ...
        self.assertEqual(
            [craft_file.library_file.filename],
            [lfa.filename for lfa in build_view.files],
        )
        # Deleted files won't be included.
        self.assertFalse(craft_file.library_file.deleted)
        removeSecurityProxy(craft_file.library_file).content = None
        self.assertTrue(craft_file.library_file.deleted)
        build_view = create_initialized_view(build, "+index")
...
class TestCraftRecipeBuildOperations(BrowserTestCase):
