...
class PublicCodehostingAPI(LaunchpadXMLRPCView):
    ...
    def _getUrlsForBranch(
        self, branch, lp_path, suffix=None, supported_schemes=None
    ):
    ...
    def _getUniqueNameResultDict(
        self, unique_name, suffix=None, supported_schemes=None, path=None
    ):
    ...
    def _resolve_lp_path(self, path):
        ...
        hot_products = [
            product.strip()
            for product in config.codehosting.hot_products.split(",")
        ]
