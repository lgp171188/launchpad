...
class CodehostingAPI(LaunchpadXMLRPCView):
    ...
    def _canWriteToBranch(self, requester, branch):
        ...
        if requester == LAUNCHPAD_SERVICES:
            return False
        return branch.branch_type == BranchType.HOSTED and check_permission(
            "launchpad.Edit", branch
        )
    ...
    def _serializeBranch(
        self, requester, branch, trailing_path, force_readonly=False
    ):
        if requester == LAUNCHPAD_SERVICES:
            branch = removeSecurityProxy(branch)
        try:
    ...
    def performLookup(self, requester, path, lookup):
        ...
        serialized = self._serializeBranch(
            requester, branch, trailing, lookup["type"] == "id"
        )
