...
class RecipientReason(basemailer.RecipientReason):
    ...
    def forReviewer(
        ...
        if pending_review:
            ...
            reason_template = "%(entity_is)s reviewing %(merge_proposal)s."
        return cls(
            reviewer,
            reviewer,
            branch_merge_proposal.merge_source,
...
class BranchMailer(BaseMailer):
    ...
    def forBranchModified(cls, branch, user, delta, delta_for_editors=None):
        ...
        for recipient in recipients:
            ...
            if (
                ...
                and subscription.notification_level not in interested_levels
            ):
                continue
    ...
    def _includeDiff(self, email):
        ...
        if reason.max_diff_lines == BranchSubscriptionDiffSize.NODIFF:
            return False
        if (
            reason.max_diff_lines != BranchSubscriptionDiffSize.WHOLEDIFF
            and self.diff_size > reason.max_diff_lines.value
        ):
            return False
        return True, ""
