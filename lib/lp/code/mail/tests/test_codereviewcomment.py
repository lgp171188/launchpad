...
class TestCodeReviewComment(TestCaseWithFactory):
    ...
    def test_generateEmail(self):
        ...
        self.assertEqual(
            ctrl.body.splitlines()[:-3], message.text_contents.splitlines()
        )
        ...
        expected = {
            "X-Launchpad-Branch": source_branch.unique_name,
    ...
    def test_useRootMessageId(self):
        ...
        self.assertEqual(
            mailer.merge_proposal.root_message_id, ctrl.headers["In-Reply-To"]
        )
    ...
    def test_nonReplyCommentUsesRootMessageId(self):
        ...
        self.assertEqual(
            comment.branch_merge_proposal.root_message_id,
    ...
    def test_generateEmailWithVote(self):
        ...
        mailer, subscriber = self.makeMailer(vote=CodeReviewVote.APPROVE)
        ctrl = mailer.generateEmail(
            subscriber.preferredemail.email, subscriber
        )
        self.assertEqual("Review: Approve", ctrl.body.splitlines()[0])
        self.assertEqual(
            ctrl.body.splitlines()[2:-3],
    ...
    def makeCommentWithInlineComments(
        self, subject=None, content=None, inline_comments=None
    ):
    ...
    def test_mailer_attachments(self):
        ...
        message = mailer.generateEmail(
            person.preferredemail.email, person
        ).makeMessage()
    ...
    def test_generateEmail_addresses(self):
        ...
        mailer = CodeReviewCommentMailer.forCreation(comment)
        ctrl = mailer.generateEmail(
            "commenter@email.com", comment.message.owner
        )
...
class TestInlineCommentsSection(testtools.TestCase):
    ...
    def test_section_header_and_footer(self):
        ...
        header = section[:4]
        self.assertEqual(["", "", "Diff comments:", ""], header)
        footer = section[-1:]
