...
def new_import(code_import, event):
    ...
    if event.user is None or IUnauthenticatedPrincipal.providedBy(event.user):
    ...
    headers = {
        "X-Launchpad-Branch": code_import.target.unique_name,
...
def make_email_body_for_code_import_update(code_import, event, new_whiteboard):
    ...
    if code_import.rcs_type == RevisionControlSystems.CVS:
        ...
        if (
            CodeImportEventDataType.OLD_CVS_ROOT in event_data
            or CodeImportEventDataType.OLD_CVS_MODULE in event_data
        ):
            old_root = event_data.get(
                CodeImportEventDataType.OLD_CVS_ROOT, code_import.cvs_root
            )
...
def code_import_updated(code_import, event, new_whiteboard, person):
    """Email target subscribers and the vcs-imports team with new status."""
