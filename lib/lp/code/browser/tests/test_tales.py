...
class TestPreviewDiffFormatter(TestCaseWithFactory):
    ...
    def test_fmt_lines_files_modified(self):
        ...
        preview = self._createPreviewDiff(
            10, added=4, removed=0, diffstat={"file1": (1, 0), "file2": (3, 0)}
        )
        self.assertEqual(
            '<a href="%s/+files/preview.diff" class="diff-link">'
            "10 lines (+4/-0)</a>"
    ...
    def test_fmt_lines_one_file_modified(self):
        ...
        preview = self._createPreviewDiff(
            10, added=4, removed=0, diffstat={"file": (3, 0)}
        )
        self.assertEqual(
            '<a href="%s/+files/preview.diff" class="diff-link">'
            "10 lines (+4/-0)</a>"
    ...
    def test_fmt_lines_escapes_file_name(self):
        ...
        preview = self._createPreviewDiff(
            10, added=4, removed=0, diffstat={"file<name>": (3, 0)}
        )
        self.assertEqual(
            '<a href="%s/+files/preview.diff" class="diff-link">'
            "10 lines (+4/-0)</a>"
    ...
    def test_fmt_stale_non_empty_diff(self):
        # If there is no diff, there is no link.
        diffstat = {self.factory.getUniqueString(): (2, 3) for x in range(23)}
