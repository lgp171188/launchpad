...
class BzrMixin:
    ...
    def checkRelatedBranches(
        ...
        root_url = canonical_url(getUtility(ILaunchpadRoot), rootsite="code")
        ...
        if not related_series_branch_info:
            ...
            for branch_info in related_series_branch_info:
                ...
                expected_branch_info.append(
                    canonical_url(
                        branch, rootsite="code", path_only_if_possible=True
                    )
                )
                expected_branch_info.append(branch.displayname)
                expected_branch_info.append(
                    canonical_url(product_series, path_only_if_possible=True)
                )
...
class TestSourcePackageRecipeAddViewInitialValuesMixin:
    ...
    def test_initial_series(self):
        ...
        experimental = self.factory.makeDistroSeries(
            distribution=archive.distribution, status=SeriesStatus.EXPERIMENTAL
        )
        ...
        frozen = self.factory.makeDistroSeries(
            distribution=archive.distribution, status=SeriesStatus.FROZEN
        )
        ...
        supported = self.factory.makeDistroSeries(
            distribution=archive.distribution, status=SeriesStatus.SUPPORTED
        )
        obsolete = self.factory.makeDistroSeries(
            distribution=archive.distribution, status=SeriesStatus.OBSOLETE
        )
        future = self.factory.makeDistroSeries(
            distribution=archive.distribution, status=SeriesStatus.FUTURE
        )
        ...
        other_series = {experimental, frozen, supported, obsolete, future}
        self.assertEqual(set(), series.intersection(other_series))


class TestSourcePackageRecipeAddViewInitialValuesBzr(
    TestSourcePackageRecipeAddViewInitialValuesMixin,
    BzrMixin,
    TestCaseWithFactory,
):
...
class TestSourcePackageRecipeAddViewMixin:
    ...
    def test_create_new_recipe(self):
        ...
        self.assertThat(
            "Edit Make some food!", MatchesTagText(content, "edit-description")
        )
        self.assertThat(
    ...
    def test_create_new_recipe_private_branch(self):
        ...
        self.assertRaises(
            LinkNotFoundError, browser.getLink, "Create packaging recipe"
        )
    ...
    def test_create_new_recipe_suggests_user_team(self):
        ...
        team = self.factory.makeTeam(
            name="branch-team", displayname="Branch Team", members=[self.chef]
        )
        branch = self.makeBranch(owner=team)
        text = self.getMainText(branch, "+new-recipe")
        self.assertTextMatchesExpressionIgnoreWhitespace(
            r"Owner: Master Chef \(chef\)"
            r" Branch Team \(branch-team\) Other:",
            text,
        )
    ...
    def test_ppa_selector_shown_if_user_has_ppas(self):
        ...
        ppa_options = list(get_radio_button_text_for_field(content, "use_ppa"))
        self.assertEqual(
            [
                "(*) Use an existing PPA",
...
class TestSourcePackageRecipeEditViewMixin:
    ...
    def test_edit_recipe(self):
        ...
        recipe = self.factory.makeSourcePackageRecipe(
            owner=self.chef,
            registrant=self.chef,
            name="things",
            description="This is a recipe",
            distroseries=self.squirrel,
            branches=[veggie_branch],
            daily_build_archive=self.ppa,
        )
    ...
    def test_admin_edit(self):
        ...
        self.assertRaises(
            LookupError, browser.getControl, name="field.daily_build_archive"
        )
...
class TestSourcePackageRecipeEditViewBzr(
    ...
    def test_edit_sourcepackage_branch_recipe_with_related_branches(self):
        ...
        with person_logged_in(self.chef):
            ...
            _, _, related_package_branch_info = self.makeRelatedBranches(
                reference_branch
            )
        browser = self.getUserBrowser(
            canonical_url(recipe, view_name="+edit"), user=self.chef
...
class TestSourcePackageRecipeViewMixin:
    ...
    def test_index_pending(self):
        ...
        self.assertTextMatchesExpressionIgnoreWhitespace(pattern, main_text)
    ...
    def test_builds(self):
        ...
        self.assertEqual(
            [build6, build5, build4, build3, build2, build1], view.builds
        )
        ...
        self.assertEqual([build4, build3, build2, build1, build6], view.builds)
        ...
        self.assertEqual([build6, build5, build4, build3, build2], view.builds)
    ...
    def test_request_daily_builds_button_no_daily_ppa(self):
        ...
        branch = self.makeBranch()
        recipe = self.factory.makeSourcePackageRecipe(
            owner=self.chef, branches=[branch], is_stale=True, build_daily=True
        )
    ...
    def test_request_daily_builds_action(self):
        ...
        build_distros = [build.distroseries.displayname for build in builds]
        build_distros.sort()
...
class TestSourcePackageRecipeBuildViewMixin:
    ...
    def makeBuild(self):
        ...
        archive = self.factory.makeArchive(name="build", owner=self.user)
        branch = self.makeBranch()
        recipe = self.factory.makeSourcePackageRecipe(
    ...
    def test_eta(self):
        ...
        self.assertEqual(
            ...
            + queue_entry.estimated_duration,
            view.eta,
        )
        queue_entry.markAsBuilding(None)
        clear_property_cache(view)
        self.assertEqual(
            queue_entry.date_started + queue_entry.estimated_duration, view.eta
        )
...
class TestSourcePackageRecipeDeleteViewMixin:
    ...
    def test_delete_recipe(self):
        ...
        self.assertEqual("http://code.launchpad.test/~chef", browser.url)
    ...
    def test_delete_recipe_no_permissions(self):
        ...
        browser = self.getUserBrowser(recipe_url, user=nopriv_person)

        self.assertRaises(LinkNotFoundError, browser.getLink, "Delete recipe")
...
class TestBrokenExistingRecipesMixin:
    ...
    def makeBrokenRecipe(self):
        ...
        recipe_text = dedent(
            ...
            """
            % (
                self.RECIPE_FIRST_LINE,
                self.getBranchRecipeText(b1),
