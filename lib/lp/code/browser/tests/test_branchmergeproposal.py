...
class TestBranchMergeProposalMergedViewMixin:
    ...
    def test_initial_values(self):
        ...
        self.assertEqual(
            self.getBranchRevisionValues(bmp.merge_target), view.initial_values
        )
    ...
    def test_notifies_modification(self):
        ...
        events = [
            ...
            and IBranchMergeProposal.providedBy(event.object)
        ]
        self.assertThat(
            events,
            MatchesListwise(
...
class TestBranchMergeProposalMergedViewGit(
    TestBranchMergeProposalMergedViewMixin,
    GitHostingClientMixin,
    BrowserTestCase,
):
...
class TestBranchMergeProposalAddVoteView(TestCaseWithFactory):
    ...
    def _createView(self):
        ...
        view = BranchMergeProposalAddVoteView(self.bmp, LaunchpadTestRequest())
        view.initialize()
        return view
...
class TestBranchMergeProposalVoteView(TestCaseWithFactory):
    ...
    def testChangeOfVoteBringsToTop(self):
        ...
        self.assertEqual(
            [albert, bob], [review.reviewer for review in view.current_reviews]
        )
    ...
    def test_render_all_vote_types(self):
        ...
        for vote in CodeReviewVote.items:
            self._createComment(self.factory.makePerson(), vote)
...
class TestRegisterBranchMergeProposalViewMixin:
    ...
    def test_register_simplest_case(self):
        ...
        view.register_action.success(
            self._getFormValues(target_branch, {"needs_review": True})
        )
    ...
    def test_register_work_in_progress(self):
        ...
        view = self._createView()
        view.register_action.success(
            self._getFormValues(target_branch, {"needs_review": False})
        )
        proposal = self._getSourceProposal(target_branch)
        self.assertEqual(
    ...
    def test_branch_visibility_notification(self):
        ...
        self.assertThat(
            ...
            MatchesRegex("To ensure visibility, .* is now subscribed to:.*"),
        )
...
class TestRegisterBranchMergeProposalViewBzr(
    ...
    def test_register_ajax_request_with_confirmation(self):
        ...
        with person_logged_in(owner):
            branches_to_check = [
                self.source_branch.unique_name,
    ...
    def test_register_ajax_request_with_validation_errors(self):
        ...
        with person_logged_in(owner):
            ...
            view = create_initialized_view(
                target_branch, name="+register-merge", request=request
            )
        self.assertEqual(
...
class TestRegisterBranchMergeProposalViewGit(
    ...
    def _makeTargetBranch(self, **kwargs):
        return self.factory.makeGitRefs(
            target=self.source_branch.target, **kwargs
        )[0]
        ...
        self.assertEqual(
            {
                ...
                "errors": {
                    "field.target_git_ref": (
                        "The target repository and path together cannot be "
                        "the same as the source repository and path."
                    )
                },
                "form_wide_errors": [],
            },
    ...
    def test_register_ajax_request_with_missing_target_git_repository(self):
        ...
        with person_logged_in(owner):
            ...
            request.setPrincipal(owner)
            view = create_initialized_view(
                self.source_branch, name="+register-merge", request=request
            )
        self.assertEqual(
            "400 Validation", view.request.response.getStatusString()
        )
        ...
        self.assertEqual(
            {
                "error_summary": "There is 1 error.",
                "errors": {
                    "field.target_git_ref": "Please enter a Git branch path."
                },
                "form_wide_errors": [],
            },
            json.loads(view.form_result),
...
class TestBranchMergeProposalRequestReviewViewGit(
    TestBranchMergeProposalRequestReviewViewMixin,
    GitHostingClientMixin,
    BrowserTestCase,
):
...
class TestBranchMergeProposalResubmitViewMixin:
    ...
    def createView(self):
        ...
        view = BranchMergeProposalResubmitView(context, LaunchpadTestRequest())
        view.initialize()
        return view
    ...
    def test_mail_jobs(self):
        ...
        self.assertThat(
            needs_review_job,
            MatchesStructure.byEquality(
                branch_merge_proposal=new_proposal, metadata={}
            ),
        )
...
class TestBranchMergeProposalResubmitViewBzr(
    ...
    @staticmethod
    def _getFormValues(
        source_branch, target_branch, prerequisite_branch, extras
    ):
        values = {
...
class TestBranchMergeProposalResubmitViewGit(
    ...
    def _getFormValues(
        ...
        if prerequisite_branch is not None:
            values.update({"prerequisite_git_ref": prerequisite_branch})
        else:
            values.update(
                {
...
class TestBranchMergeProposalView(TestCaseWithFactory):
    ...
    def test_claim_action_non_member(self):
        ...
        view = create_initialized_view(self.bmp, "+index")
        self.assertRaises(
            Unauthorized, view.claim_action.success, {"review_id": review.id}
        )
    ...
    def test_preview_diff_all_chars(self):
        ...
        view = create_initialized_view(self.bmp, "+index")
        self.assertEqual(
            diff.decode("windows-1252", "replace"), view.preview_diff_text
        )
        self.assertTrue(view.diff_available)
    ...
    def test_getRevisionsIncludesIncrementalDiffs(self):
        ...
        self.assertEqual([diff], [comment.diff for comment in comments])
    ...
    def test_include_superseded_comments(self):
        ...
        self.assertEqual(
            ...
            [
                comment.comment.message.subject
                for comment in view.conversation.comments
            ],
        )
    ...
    def test_description_is_meta_description(self):
        description = (
            ...
            "description: this does that " + "abcdef " * 300
        )
        bmp = self.factory.makeBranchMergeProposal(description=description)
        browser = self.getUserBrowser(canonical_url(bmp, rootsite="code"))
        expected_meta = Tag(
            "meta description",
            "meta",
            attrs=dict(name="description", content=description[:497] + "..."),
        )
    ...
    def test_revisionStatusReports_display(self):
        ...
        with person_logged_in(owner):
            ...
            self.assertThat(
                reports_section[0],
                Tag("first report summary", "td", text=report1.result_summary),
            )
            self.assertThat(
                reports_section[0],
                Within(
...
class TestBranchMergeProposalBrowserView(BrowserTestCase):
    ...
    def test_merge_guidelines_package(self):
        ...
        [source_ref] = self.factory.makeGitRefs(
            repository=source_repository, paths=["refs/heads/branch"]
        )
        ...
        git_add_remote_match = HTMLContains(
            Tag(
                ...
                text=(
                    "git remote add %s "
                    "git+ssh://%s@git.launchpad.test/~%s/%s/+source/choc/+git/"
                    "%s"
                )
                % (
                    source_owner.name,
    ...
    def test_merge_guidelines_anonymous_view(self):
        ...
        with person_logged_in(other_user):
            view = create_initialized_view(bmp, "+index", principal=other_user)
        git_add_remote_match = HTMLContains(
            Tag("Git remote add text", "tt", attrs={"id": "remote-add"})
        )
        git_remote_update_match = HTMLContains(
            Tag("Git remote update text", "tt", attrs={"id": "remote-update"})
        )
        git_checkout_match = HTMLContains(
            Tag(
                "Checkout command text",
        ...
        git_merge_match = HTMLContains(
            Tag("Merge command text", "tt", attrs={"id": "merge-cmd"})
        )
    ...
    def test_merge_guidelines_personal(self):
        ...
        [other_ref] = self.factory.makeGitRefs(
            repository=other_repository, paths=["refs/heads/branch"]
        )
        self.assertRaises(
...
class TestBranchMergeProposalChangeStatusView(TestCaseWithFactory):
    ...
    def assertStatusVocabTokens(self, tokens, user):
        ...
        self.assertEqual(sorted(tokens), vocab_tokens)
    ...
    def test_createStatusVocabulary_non_reviewer(self):
        ...
        status_options = ["WORK_IN_PROGRESS", "NEEDS_REVIEW", "MERGED"]
    ...
    def test_createStatusVocabulary_rejected(self):
        ...
        self.assertAllStatusesAvailable(
            user=self.proposal.registrant, except_for=["CODE_APPROVED"]
        )
        self.assertAllStatusesAvailable(user=self.proposal.target_branch.owner)
...
class TestBranchMergeProposal(BrowserTestCase):
    ...
    def test_conversation(self):
        ...
        parent = add_revision_to_branch(
            self.factory, source_branch, self.factory.getUniqueDate()
        ).revision
        bmp = self.factory.makeBranchMergeProposal(
            registrant=self.user,
    ...
    def test_short_conversation_comments_no_download(self):
        ...
        browser = self.getViewBrowser(comment.branch_merge_proposal)
        body = Tag(
            "Download", "a", {"href": download_url}, text="Download full text"
        )
    ...
    def test_show_rescan_link_bzr(self):
        bmp = self.factory.makeBranchMergeProposal()
        target_job = getUtility(IBranchScanJobSource).create(bmp.target_branch)
        removeSecurityProxy(target_job).job._status = JobStatus.FAILED
        view = create_initialized_view(bmp, "+index")
        self.assertTrue(view.show_rescan_link)
    ...
    def test_opengraph_description_is_commit(self):
        ...
        self.assertEqual(view.page_description, bmp.description)
...
class TestBranchMergeProposalRescanView(BrowserTestCase):
    ...
    def test_rescan_with_git(self):
        ...
        with person_logged_in(bmp.merge_source.owner):
            ...
            view = create_initialized_view(
                bmp, name="+rescan", request=request
            )

        self.assertEqual(
            "Rescan scheduled", view.request.response.notifications[0].message
        )

    def test_rescan_with_bzr(self):
        bmp = self.factory.makeBranchMergeProposal()
        source_job = getUtility(IBranchScanJobSource).create(bmp.source_branch)
        removeSecurityProxy(source_job).job._status = JobStatus.FAILED
...
class TestBranchMergeProposalLinkBugViewGit(
    TestBranchMergeProposalLinkBugViewMixin,
    GitHostingClientMixin,
    BrowserTestCase,
):
    def _makeBranchMergeProposal(self):
...
class TestBranchMergeProposalDeleteViewGit(
    TestBranchMergeProposalDeleteViewMixin,
    GitHostingClientMixin,
    BrowserTestCase,
):
