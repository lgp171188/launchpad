...
from lp.testing.matchers import Contains, HasQueryCount
...
class TestGitRepositoryNavigation(TestCaseWithFactory):
    ...
    def test_traverse_artifact(self):
        ...
        report = self.factory.makeRevisionStatusReport(git_repository=repo)
...
class TestGitRepositoryView(BrowserTestCase):
    ...
    def test_landing_candidates_query_count(self):
        ...
        recorder1, recorder2 = record_two_runs(
            login_and_view, create_merge_proposal, 2
        )
    ...
    def test_view_with_landing_targets(self):
        ...
        [target_git_ref] = self.factory.makeGitRefs(
            target_repository, paths=["refs/heads/master"]
        )
    ...
    def test_landing_targets_query_count(self):
        ...
        def create_merge_proposal():
            [target_git_ref] = self.factory.makeGitRefs(target_repository)
            [source_git_ref] = self.factory.makeGitRefs(source_repository)
            bmp = self.factory.makeBranchMergeProposalForGit(
                target_ref=target_git_ref,
                source_ref=source_git_ref,
                set_state=BranchMergeProposalStatus.NEEDS_REVIEW,
...
class TestGitRepositoryRescanView(BrowserTestCase):
    ...
    def test_owner_can_see_rescan(self):
        ...
        job = GitRefScanJob.create(repository)
        job.job._status = JobStatus.FAILED
        url = canonical_url(repository, view_name="+rescan", rootsite="code")
    ...
    def test_admin_can_see_rescan(self):
        ...
        browser = self.getUserBrowser(
            url, user=self.factory.makeCommercialAdmin()
        )
        browser.open(url)
        self.assertIn("schedule a rescan", browser.contents)
    ...
    def test_other_user_can_not_see_rescan(self):
        ...
        job.job._status = JobStatus.FAILED
        url = canonical_url(repository, view_name="+rescan", rootsite="code")
        self.assertRaises(Unauthorized, self.getUserBrowser, url)
...
class TestGitRepositoryEditView(TestCaseWithFactory):
    ...
    def test_repository_default_branch_widget_required(self):
        ...
        login_person(person)
        view = create_initialized_view(repository, name="+edit")
        self.assertFalse(view.widgets["default_branch"].context.required)

        removeSecurityProxy(repository)._default_branch = "refs/heads/master"
        view = create_initialized_view(repository, name="+edit")
        self.assertTrue(view.widgets["default_branch"].context.required)
...
class TestGitRepositoryDiffView(BrowserTestCase):
    ...
    def test_render(self):
        ...
        hosting_fixture = self.useFixture(
            GitHostingFixture(diff={"patch": diff})
        )
        person = self.factory.makePerson()
        repository = self.factory.makeGitRepository(
...
class TestGitRepositoryPermissionsView(BrowserTestCase):
    ...
    def test__getFieldName_no_grantee(self):
        ...
        view = create_initialized_view(repository, name="+permissions")
        encoded_ref_pattern = (
            base64.b32encode(b"refs/heads/*").decode("UTF-8").replace("=", "_")
        )
        self.assertEqual(
    ...
    def test__getFieldName_grantee_repository_owner(self):
        ...
        view = create_initialized_view(repository, name="+permissions")
        encoded_ref_pattern = (
            base64.b32encode(b"refs/tags/*").decode("UTF-8").replace("=", "_")
        )
        self.assertEqual(
            "field.%s._repository_owner" % encoded_ref_pattern,
            view._getFieldName(
                "field", "refs/tags/*", grantee=GitGranteeType.REPOSITORY_OWNER
            ),
        )
    ...
    def test__getFieldName_grantee_person(self):
        ...
        view = create_initialized_view(repository, name="+permissions")
        encoded_ref_pattern = (
            base64.b32encode(b"refs/*").decode("UTF-8").replace("=", "_")
        )
        self.assertEqual(
    ...
    def test__getPermissionsTerm_standard(self):
        ...
        self.assertThat(
            view._getPermissionsTerm(grant),
            MatchesStructure.byEquality(
                value={
    ...
    def _matchesRuleGrant(
        self, pattern, grantee, permissions_token, permissions_title
    ):
        ...
        self.assertThat(
            ...
            MatchesListwise(
                [
                    ...
                    self._matchesNewRuleGrant(
                        "refs/heads/stable/*", "can_push"
                    ),
    ...
    def assertHasSavedNotification(self, view, repository):
        self.assertThat(
            ...
            MatchesListwise(
                [
                    MatchesStructure.byEquality(
                        message="Saved permissions for %s"
                        % repository.identity
                    ),
                ]
            ),
    ...
    def test_save_add_rules(self):
        ...
        self.assertThat(
            list(repository.getActivity()),
            MatchesListwise(
                [
                    # Adding a tag rule automatically adds a repository owner
                    # grant.
                    MatchesStructure(
                        ...
                        new_value=MatchesDict(
                            {
                                ...
                                # Initially inserted at 1, although
                                # refs/tags/1.0 was later inserted before it.
                                "position": Equals(1),
                            }
                        ),
                    ),
        ...
        self.assertThat(
            ...
            MatchesListwise(
                [
                    MatchesStructure(
                        field_name=Equals(
                            "new-pattern." + encoded_heads_prefix
                        ),
                        errors=MatchesStructure.byEquality(
                            args=(
                                "stable/* is already in use by another rule",
                            )
                        ),
                    ),
                ]
    ...
    def test_save_move_rule(self):
        ...
        encoded_patterns = [
            encode_form_field_id(rule.ref_pattern) for rule in repository.rules
        ]
        removeSecurityProxy(repository.getActivity()).remove()
        login_person(repository.owner)
                ...
                [
                    MatchesStructure(
                        ...
                        new_value=MatchesDict(
                            {
                                "ref_pattern": Equals(
                                    "refs/heads/*/more-next"
                                ),
                                "position": Equals(0),
                            }
                        ),
                    ),
                    # Only one rule is recorded as moving; the other is
                    # already in its new position by the time it's
                    # processed.
                    MatchesStructure(
                        changer=Equals(repository.owner),
    ...
    def test_save_change_grants(self):
        ...
        login_person(repository.owner)
        form = {
            "field.permissions.%s._repository_owner"
            % encoded_patterns[0]: ("can_push"),
...
class TestGitRepositoryActivityView(BrowserTestCase):
    ...
    def test_render(self):
        ...
        browser = self.getViewBrowser(
            repository, "+activity", rootsite="code", user=repository.owner
        )

        table = find_tag_by_id(browser.contents, "activity-listing")
        date = repository.getActivity()[0].date_changed.strftime("%Y-%m-%d %T")
        ...
        self.assertThat(
            ...
            Rule position: 0
            """.format(
                        date=date, person_name=person_name
                    )
                ),
                flags=doctest.NORMALIZE_WHITESPACE,
            ),
    ...
    def test_activity_query_count(self):
        ...
        recorder1, recorder2 = record_two_runs(
            login_and_view, create_activity, 2
        )
        self.assertThat(recorder2, HasQueryCount.byEquality(recorder1))
    ...
    def test_fork_page_submit_missing_user(self):
        ...
        with person_logged_in(another_person):
            view = create_initialized_view(
                repository,
                name="+fork",
                form={"field.actions.fork": "Fork it"},
            )
