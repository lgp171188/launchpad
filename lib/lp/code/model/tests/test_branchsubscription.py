...
class TestBranchSubscriptions(TestCaseWithFactory):
    ...
    def test_subscribe_with_editable_stacked_branch(self):
        ...
        with person_logged_in(owner):
            ...
            self.assertIn(grantee, private_stacked_on_branch.subscribers)
    ...
    def test_subscribe_with_non_editable_stacked_branch(self):
        ...
        product = self.factory.makeProduct(owner=owner)
        private_stacked_on_branch = self.factory.makeBranch(
            product=product, information_type=InformationType.PRIVATESECURITY
        )
        branch = self.factory.makeBranch(
            product=product,
            owner=owner,
        ...
        with person_logged_in(owner):
            ...
            self.assertIn(grantee, branch.subscribers)
            self.assertNotIn(grantee, private_stacked_on_branch.subscribers)
    ...
    def test_subscribe_with_public_stacked_branch(self):
        ...
        with person_logged_in(owner):
            ...
            self.assertIn(grantee, branch.subscribers)
            self.assertNotIn(grantee, public_stacked_on_branch.subscribers)
