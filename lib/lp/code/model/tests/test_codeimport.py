...
class TestConsecutiveFailureCount(TestCodeImportBase):
    ...
    def succeedImport(
        self, code_import, status=CodeImportResultStatus.SUCCESS
    ):
        ...
        running_job = self.makeRunningJob(code_import)
        getUtility(ICodeImportJobWorkflow).finishJob(running_job, status, None)
...
class TestRequestImport(TestCodeImportBase):
    ...
    def test_exception_on_disabled(self):
        ...
        self.assertRaises(
            CodeImportNotInReviewedState, code_import.requestImport, requester
        )
    ...
    def test_exception_if_already_running(self):
        ...
        code_import = make_running_import(
            factory=self.factory, code_import=code_import
        )
        requester = self.factory.makePerson()
        self.assertRaises(
            CodeImportAlreadyRunning, code_import.requestImport, requester
        )
...
class TestCodeImportWebservice(TestCodeImportBase):
    ...
    def test_codeimport_owner_can_set_url(self):
        ...
        response = webservice.patch(
            code_import_url, "application/json", json.dumps({"url": new_url})
        )
