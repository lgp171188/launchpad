...
from lp.registry.browser.pillar import InvolvedMenu, PillarInvolvementView
...
class ProductSeriesInvolvedMenu(InvolvedMenu):
    ...
    links = ["report_bug", "help_translate", "register_blueprint"]
...
class ProductSeriesSpecificationsMenu(
...
class ProductSeriesView(
    ...
    def golang_import_spec(self):
        ...
        """
        if (
            self.context.product.vcs == VCSType.BZR
            and self.user_branch_visible
        ):
            return (
                "{hostname}/{product}/{series} bzr {root_url}{branch}"
            ).format(
    ...
    def user_branch_visible(self):
        ...
        return branch is not None and check_permission(
    ...
    def bugtask_status_counts(self):
        ...
        return [
            StatusCount(status, status_counts[status]) for status in statuses
        ]
    ...
    def specification_status_counts(self):
        ...
        return [
            ...
            for status in sorted(status_counts, key=attrgetter("sortkey"))
        ]
...
class ProductSeriesDeleteView(RegistryDeleteViewMixin, LaunchpadEditFormView):
    ...
    def can_delete(self):
        ...
        return not (
            ...
            or self.has_linked_packages
            or self.has_translations
        )
