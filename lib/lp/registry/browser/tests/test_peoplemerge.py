...
class RequestPeopleMergeMixin(TestCaseWithFactory):
    def setUp(self):
        ...
        self.dupe = self.factory.makePerson(name="foo", email="foo@baz.com")
...
class TestRequestPeopleMergeMultipleEmails(RequestPeopleMergeMixin):
    ...
    def _assert_perform_merge_request(self):
        ...
        browser.getControl("Duplicated Account").value = "foo"
        browser.getControl("Continue").click()
        explanation = find_tag_by_id(browser.contents, "explanation")
        self.assertThat(
            extract_text(explanation),
            DocTestMatches(
                "The account..."
...
class TestRequestPeopleMergeSingleEmail(RequestPeopleMergeMixin):
    ...
    def _perform_merge_request(self, dupe):
        ...
        browser.getControl("Duplicated Account").value = dupe_name
        browser.getControl("Continue").click()
        return browser
...
class TestValidatingMergeView(TestCaseWithFactory):
        ...
        self.assertEqual(
            [
                html_escape(
                    ...
                    "merged. It may take ten minutes to remove the deleted "
                    "PPA's files."
                )
            ],
            view.errors,
    ...
    def test_cannot_merge_dupe_person_with_an_existing_merge_job(self):
        ...
        job_source.create(
            from_person=self.dupe,
            to_person=self.target,
        ...
        self.assertEqual(["dupe is already queued for merging."], view.errors)
...
class TestRequestPeopleMergeMultipleEmailsView(TestCaseWithFactory):
    ...
    def setUp(self):
        ...
        self.email_2 = self.factory.makeEmail("dupe@place.dom", self.dupe_user)
...
class TestAdminTeamMergeView(TestCaseWithFactory):
    ...
    def test_cannot_merge_team_with_ppa(self):
        ...
        self.assertEqual(
            [
                html_escape(
                    "dupe-team has a PPA that must be deleted before it can "
