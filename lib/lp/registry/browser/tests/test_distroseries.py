...
from testtools.matchers import EndsWith, LessThan, Not
...
class DistroSeriesIndexFunctionalTestCase(TestCaseWithFactory):
    ...
    def test_differences_portlet_all_differences(self):
        ...
        portlet_display = soupmatchers.HTMLContains(
            soupmatchers.Tag(
                "Derivation portlet header", "h2", text="Derived from Sid"
            )
        )
    ...
    def test_differences_portlet_initialization_failed(self):
        ...
        portlet_display = soupmatchers.HTMLContains(
            soupmatchers.Tag(
                "Derived series", "h2", text="Series initialization has failed"
            ),
        )
        with person_logged_in(self.simple_user):
            view = create_initialized_view(
...
class TestMilestoneBatchNavigatorAttribute(TestCaseWithFactory):
    ...
    def _check_milestone_batch_navigator(self, view):
        ...
        milestone_names = [
            item.name for item in view.milestone_batch_navigator.currentBatch()
        ]
...
class TestDistroSeriesAddView(TestCaseWithFactory):
    ...
    def createNewDistroseries(self):
        ...
        with person_logged_in(self.user):
            create_initialized_view(self.distribution, "+addseries", form=form)
    ...
    def test_submit_sets_previous_series(self):
        ...
        # A yet older series.
        self.factory.makeDistroSeries(self.distribution, version="11.04")
...
class TestDistroSeriesInitializeView(TestCaseWithFactory):
    ...
    def test_form_shown(self):
        ...
        self.assertIn("Javascript is required to use this page", message.text)
        self.assertIn("javascript-disabled", message.get("class").split())
    ...
    def test_seriesToVocab(self):
        ...
        self.assertEqual(
            ["api_uri", "title", "value"], sorted(formatted_dict.keys())
        )
    ...
    def test_not_is_first_derivation(self):
        ...
        self.assertContentEqual(
            seriesToVocab(previous_series), cache["previous_series"]
        )
        self.assertEqual(2, len(cache["previous_parents"]))
        self.assertContentEqual(
            seriesToVocab(previous_parent1), cache["previous_parents"][0]
        )
        self.assertContentEqual(
            seriesToVocab(previous_parent2), cache["previous_parents"][1]
        )
    ...
    def test_form_hidden_when_previous_series_none(self):
        ...
        distroseries = self.factory.makeDistroSeries(previous_series=None)
        another_distroseries = self.factory.makeDistroSeries(
            distribution=distroseries.distribution
        )
        self.factory.makeSourcePackagePublishingHistory(
...
class TestDistroSeriesInitializeViewAccess(TestCaseWithFactory):
    ...
    def test_initseries_access_anon(self):
        ...
        self.assertEqual(False, check_permission("launchpad.Edit", view))
    ...
    def test_initseries_access_admin(self):
        ...
        self.assertEqual(True, check_permission("launchpad.Edit", view))
...
class DistroSeriesDifferenceMixin:
    ...
    def _test_packagesets(
        self, html_content, packageset_text, packageset_class, msg_text
    ):
...
class TestDistroSeriesLocalDiffPerformance(
    TestCaseWithFactory, DistroSeriesDifferenceMixin
):
    ...
    def _assertQueryCount(self, derived_series):
        ...
        ArchivePermission(
            archive=derived_series.main_archive,
            person=self.simple_user,
        ...
        def add_differences(num):
            for index in range(num):
                ...
                if index % 2 == 0:
                    naked_spr.source_package_recipe_build = (
        ...
        self.addDetail(
            "statement-diff",
            Content(UTF8_TEXT, statement_differ(recorder2, recorder3)),
        )
        # Details about the number of statements per row.
        statement_count_per_row = (recorder3.count - recorder1.count) / float(
            batch_size
        )
        self.addDetail(
...
class TestDistroSeriesLocalDifferences(
    ...
    def makeView(self, distroseries=None):
        ...
        if distroseries is None:
            distroseries = self.factory.makeDistroSeriesParent().derived_series
    ...
    def test_filter_form_if_differences(self):
        ...
        derived_series, parent_series = self._createChildAndParent()
        self.factory.makeDistroSeriesDifference(derived_series=derived_series)
    ...
    def test_parent_packagesets_localpackagediffs(self):
        ...
        self._test_packagesets(
            html_content, packageset_text, "packagesets", "Packagesets"
        )
    ...
    def test_label_multiple_parents(self):
        ...
        view = create_initialized_view(derived_series, "+localpackagediffs")
    ...
    def test_multiple_parents_display(self):
        ...
        multiple_parents_matches = soupmatchers.HTMLContains(
            soupmatchers.Tag(
                "Parent table header", "th", text=re.compile(r"\s*Parent\s")
            ),
            soupmatchers.Tag(
                "Parent version table header",
                "th",
    ...
    def test_diff_row_last_changed(self):
        ...
        root = html.fromstring(view())
        [creator_cell] = root.cssselect("table.listing tbody td.last-changed")
        self.assertIn(
            "by %s" % (dsd.source_package_release.creator.displayname,),
    ...
    def enableDerivedSeriesUpgradeFeature(self):
        ...
        self.useFixture(
            FeatureFixture({"soyuz.derived_series_upgrade.enabled": "on"})
        )
    ...
    def test_upgrade_creates_sync_jobs(self):
        ...
        jobs = list(job_source.getActiveJobs(series.distribution.main_archive))
    ...
    def test_higher_radio_mentions_parent(self):
        ...
        radio_option_matches = soupmatchers.HTMLContains(
            soupmatchers.Tag(
                "radio displays parent's name", "label", text=radio_title
            ),
        )
    ...
    def _setUpDSD(
        ...
        sourcepackagename = self.factory.getOrMakeSourcePackageName(src_name)
    ...
    def test_describeJobs_reports_pending_queue(self):
        ...
        expected = (
            ...
            % (
                canonical_url(dsd.derived_series),
                pu.status.value,
                pu.status.name,
            )
        )
    ...
    def test_describeJobs_reports_pending_sync_and_update(self):
        ...
        self.factory.makePackageUpload(
            distroseries=dsd.derived_series, package_copy_job=pcj.id
        )
        get_property_cache(view).pending_syncs = {
            dsd.source_package_name.name: pcj,
        }
    ...
    def test_sync_error_nothing_selected(self):
        ...
        self.assertEqual("No differences selected.", view.errors[0])
    ...
    def test_sync_error_invalid_selection(self):
        ...
        view = self._syncAndGetView(derived_series, person, [another_id])

        self.assertEqual(2, len(view.errors))
        self.assertEqual("No differences selected.", view.errors[0])
        self.assertEqual("Invalid value", view.errors[1].error_name)
    ...
    def test_sync_error_no_perm_dest_archive(self):
        ...
        person = self._setUpPersonWithPerm(derived_series)
        view = self._syncAndGetView(derived_series, person, [diff_id])
    ...
    def test_sync_with_sponsoring(self):
        ...
        self._syncAndGetView(
            derived_series, person, [diff_id], sponsored=sponsored_person
        )
    ...
    def test_sync_success(self):
        ...
        view = self._syncAndGetView(
            ...
            person,
            [diff_id],
            query_string=("batch=12&start=24&my-old-man=dustman"),
        )
    ...
    def test_sync_success_not_yet_in_derived_series(self):
        ...
        view = self._syncAndGetView(
            derived_series, person, [diff_id], view_name="+missingpackages"
        )
    ...
    def test_sync_in_released_series_in_updates(self):
        ...
        self._syncAndGetView(derived_series, person, [diff_id])
    ...
    def test_diff_view_action_url(self):
        ...
        self.assertEqual("http://127.0.0.1?start=1&batch=1", view.action_url)
    ...
    def test_specified_packagesets_filter_specified(self):
        ...
        with person_logged_in(person):
            ...
            self.assertContentEqual(
                [packageset1, packageset2], view.specified_packagesets_filter
            )
    ...
    def test_specified_changed_by_filter_specified(self):
        ...
        with person_logged_in(person):
            ...
            self.assertContentEqual(
                [changed_by1, changed_by2], view.specified_changed_by_filter
            )
...
class DistroSeriesMissingPackageDiffsTestCase(TestCaseWithFactory):
    ...
    def test_missingpackages_differences(self):
        ...
        view = create_initialized_view(derived_series, "+missingpackages")

        self.assertContentEqual([missing_diff], view.cached_differences.batch)
...
class DistroSeriesMissingPackagesPageTestCase(
    TestCaseWithFactory, DistroSeriesDifferenceMixin
):
...
class DistroSerieUniquePackageDiffsTestCase(
    TestCaseWithFactory, DistroSeriesDifferenceMixin
):
    ...
    def test_uniquepackages_differences(self):
        ...
        view = create_initialized_view(derived_series, "+uniquepackages")
...
class DistroSeriesUniquePackagesPageTestCase(
    TestCaseWithFactory, DistroSeriesDifferenceMixin
):
...
class TestDistroSeriesEditView(TestCaseWithFactory):
    ...
    def test_edit_full_functionality_sets_datereleased(self):
        ...
        distroseries = self.factory.makeDistroSeries(distribution=ubuntu)
        form = {"field.actions.change": "Change", "field.status": "CURRENT"}
