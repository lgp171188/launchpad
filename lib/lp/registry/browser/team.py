...
from zope.formlib.form import Fields, FormField, FormFields
...
from zope.schema import Bool, Choice, List, Text
...
from lp.registry.browser.teamjoin import TeamJoinMixin, userIsActiveTeamMember
...
from lp.services.webapp.authorization import check_permission, clear_cache
    ...
    * The user has a current commercial subscription.
    ...
    def validate(self, data):
        ...
        if visibility != PersonVisibility.PUBLIC:
            ...
            if data["membership_policy"] != TeamMembershipPolicy.RESTRICTED:
                self.setFieldError(
                    "membership_policy",
...
class TeamEditView(
    TeamFormMixin, PersonRenameFormMixin, HasRenewalPolicyMixin
):
    ...
    def setUpWidgets(self):
        ...
        try:
            ...
            self.widgets["membership_policy"].vocabulary = SimpleVocabulary(
                [
            ...
            self.widgets["membership_policy"].extra_hint_class = "sprite info"
...
class TeamContactAddressView(MailingListTeamBaseView):
    ...
    def getContactMethodField(self):
        ...
        if self.list_is_usable:
            ...
            title = structured(
                ...
                "<strong>%s</strong>",
                self.mailinglist_address,
            )
            hosted_list_term = SimpleTerm(
                TeamContactMethod.HOSTED_LIST,
                TeamContactMethod.HOSTED_LIST.name,
                title,
            )
    ...
    def initial_values(self):
        ...
        return dict(
            contact_address=context.preferredemail.email,
...
class TeamMailingListConfigurationView(MailingListTeamBaseView):
    ...
    def save_action(self, action, data):
        ...
        assert (
            self.mailing_list is not None and self.mailing_list.is_usable
    ...
    @action(
        "Deactivate this Mailing List",
        name="deactivate_list",
    ...
    @action(
        "Reactivate this Mailing List",
        name="reactivate_list",
    ...
    def list_is_usable_but_not_contact_method(self):
        ...
        return self.list_is_usable and (
            self.context.preferredemail is None
    ...
    def mailing_list_status_message(self):
        ...
        if (
            self.mailing_list is None
            or self.mailing_list.status == MailingListStatus.PURGED
        ):
            ...
            return _(
                "An update to this team's mailing list is pending "
                "and has not yet taken effect."
            )
        elif self.mailing_list.status == MailingListStatus.UPDATING:
            return _(
                "A change to this team's mailing list is currently "
                "being applied."
            )
...
class TeamMailingListModerationView(MailingListTeamBaseView):
    ...
    def moderate_action(self, action, data):
        ...
        for field_name in form:
            if field_name.startswith("field.") and field_name.endswith(""):
                # A moderated message.
...
class ProposedTeamMembersEditView(LaunchpadFormView):
    ...
    def action_save(self, action, data):
        ...
        if len(failed_joins) > 0:
            ...
            mapping = dict(
                this_team=target_team.displayname, failed_list=failed_list
            )

            if len(failed_joins) == 1:
                self.request.response.addInfoNotification(
                    _(
                        "${this_team} is a member of the following team, so "
                ...
                self.request.response.addInfoNotification(
                    _(
                        "${this_team} is a member of the following teams, so "
...
class ITeamMember(Interface):
    ...
    newmember = PersonChoice(
        ...
        description=_(
            "The user or team which is going to be "
...
class TeamMemberAddView(LaunchpadFormView):
    ...
    def add_action(self, action, data):
        ...
        changed, new_status = self.context.addMember(
            newmember, reviewer=self.user, status=TeamMembershipStatus.APPROVED
        )
...
class TeamMembershipSelfRenewalView(LaunchpadFormView):
    ...
    def reason_for_denied_renewal(self):
        ...
        if context.status not in (admin, approved):
            ...
            text = (
                '<a href="%s">%s</a> is not a team that allows its '
            ...
            text = (
                "it is not set to expire in %d days or less. "
...
class TeamInvitationView(LaunchpadFormView):
    ...
    def page_title(self):
        return (
            smartquote('"%s" team invitation') % self.context.team.displayname
        )
    ...
    def accept_action(self, action, data):
        ...
        try:
            ...
            self.request.response.addInfoNotification(
                _(
                    "This team may not be added to ${that_team} because it is "
            ...
            self.request.response.addInfoNotification(
                _(
                    "This team is now a member of ${team}.",
                    mapping=dict(team=self.context.team.displayname),
                )
            )
    ...
    def decline_action(self, action, data):
        ...
        self.request.response.addInfoNotification(
            _(
                "Declined the invitation to join ${team}",
                mapping=dict(team=self.context.team.displayname),
            )
        )
...
class TeamMenuMixin(PPANavigationMenuMixIn, CommonMenuLinks):
    ...
    def add_poll(self):
        target = "+newpoll"
        text = "Create a poll"
        enabled = check_permission(
            "launchpad.Edit", self.context
        ) and check_permission("launchpad.AnyLegitimatePerson", self.context)
        return Link(target, text, icon="add", enabled=enabled)
    ...
    def join(self):
        ...
        if userIsActiveTeamMember(person):
            enabled = False
        elif self.person.membership_policy == TeamMembershipPolicy.RESTRICTED:
            # This is a restricted team; users can't join.
            enabled = False
        target = "+join"
        text = "Join the team"
...
class TeamIndexView(PersonIndexView, TeamJoinMixin):
    ...
    def super_teams(self):
        ...
        return [
            team
            for team in self.context.super_teams
    ...
    def can_show_subteam_portlet(self):
        ...
        """
        try:
            return len(self.super_teams) > 0 or (
                self.context.open_membership_invitations
...
class TeamJoinView(LaunchpadFormView, TeamJoinMixin):
    ...
    def join_allowed(self):
        ...
        if not (
            self.context.visibility is None
    ...
    def user_wants_list_subscriptions(self):
        ...
        return (
            self.user.mailing_list_auto_subscribe_policy
        ...
        if self.user_can_request_to_join:
            ...
            if self.direct_team_membership_requires_approval:
                response.addInfoNotification(
                    _(
                        "Your request to join ${team} is awaiting "
                        "approval.",
    ...
    def _subscribeToList(self, response):
        ...
        if self.user_can_subscribe_to_list:
            ...
            if self.direct_team_membership_requires_approval:
                response.addInfoNotification(
                    _(
                        "Your mailing list subscription is "
                        "awaiting approval."
                    )
                )
            else:
