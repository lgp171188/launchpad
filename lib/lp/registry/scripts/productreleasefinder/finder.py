...
from lp.services.timeout import default_timeout, urlfetch
...
class ProductReleaseFinder:
        ...
        for product_name, series_name, glob in found_globs:
            if last_product and last_product != product_name:
                self.log.info(
                    ...
                    last_product,
                    len(products_with_filters[last_product]),
                )
    ...
    def handleProduct(self, product_name, filters):
        """Scan for tarballs and create ProductReleases for this product."""
        ...
        for series_name, url in hose:
            if series_name is not None:
                try:
                    ...
                    self.log.exception(
                        "Could not successfully process " "URL %s for %s/%s",
                        url,
                        product_name,
                        series_name,
                    )
                ...
                self.log.debug(
                    "File in %s found that matched no glob: %s",
                    product_name,
                    url,
                )
    ...
    def handleRelease(self, product_name, series_name, url, file_names):
        ...
        if not sane_version(version):
            self.log.error(
                "Version number '%s' for '%s' is not sane", version, url
            )
            return
