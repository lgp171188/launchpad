...
def most_recent_publications(dsds, in_parent, statuses, match_version=False):
    ...
    distinct_on = (DistroSeriesDifference.source_package_name_id,)
    store = IStore(SourcePackagePublishingHistory)
    return (
        store.find(columns, conditions)
        .order_by(*order_by)
        .config(distinct=distinct_on)
    )
...
def get_packagesets(dsds, in_parent):
    ...
    results = tables.find(
        ...
        == (
            ...
            if in_parent
            else DistroSeriesDifference.derived_series_id
        ),
...
def eager_load_dsds(dsds):
    ...
    for dsd in dsds:
        ...
        if spn_id in source_pubs_for_release:
            ...
            cache.source_package_release = DistributionSourcePackageRelease(
                dsd.derived_series.distribution, spph.sourcepackagerelease
            )
        else:
        ...
        if spn_id in parent_source_pubs_for_release:
            ...
            cache.parent_source_package_release = (
                DistributionSourcePackageRelease(
                    dsd.parent_series.distribution, spph.sourcepackagerelease
                )
            )
        else:
    ...
    sprbs = bulk.load_related(
        SourcePackageRecipeBuild, sprs, ("source_package_recipe_build_id",)
    )
...
def get_comment_with_status_change(status, new_status, comment):
    ...
    new_comment = "Ignored: %s => %s" % (status.title, new_status.title)
    if comment:
...
class DistroSeriesDifference(StormBase):
    ...
    package_diff_id = Int(name="package_diff", allow_none=True)
    package_diff = Reference(package_diff_id, "PackageDiff.id")

    parent_package_diff_id = Int(name="parent_package_diff", allow_none=True)
    parent_package_diff = Reference(parent_package_diff_id, "PackageDiff.id")

    status = DBEnum(
        name="status", allow_none=False, enum=DistroSeriesDifferenceStatus
    )
    difference_type = DBEnum(
    ...
    def title(self):
        ...
        return (
            "Difference between distroseries '%(parent_name)s' and "
    ...
    def source_package_release(self):
        return self._package_release(self.derived_series, self.source_version)
    ...
    def base_distro_source_package_release(self):
        """See `IDistroSeriesDifference`."""
        return DistributionSourcePackageRelease(
            self.parent_series.distribution, self.parent_source_package_release
        )
    ...
    def _updateVersionsAndStatus(self, manual):
        ...
        if self.parent_source_pub:
            ...
            if (
                self.parent_source_version is None
                or apt_pkg.version_compare(
    ...
    def _getCommentWithStatusChange(self, new_status, comment=None):
        return get_comment_with_status_change(self.status, new_status, comment)
    ...
    def requestPackageDiffs(self, requestor):
        """See `IDistroSeriesDifference`."""
        if (
            self.base_source_pub is None
            or self.source_pub is None
            or self.parent_source_pub is None
        ):
