...
class TestTeamMembershipSetScripts(TestCaseWithFactory):
    ...
    def test_handleMembershipsExpiringToday_permissions(self):
        ...
        self.assertEqual(TeamMembershipStatus.EXPIRED, teammembership.status)
...
class TestTeamMembershipSet(TestCaseWithFactory):
    ...
    def test_getMembershipsExpiringOnDates(self):
        ...
        membership1 = self.addToTeamSetExpiryDate(member1, team, datetime1)
        member2 = self.factory.makePerson()
        membership2 = self.addToTeamSetExpiryDate(
            member2, team, datetime2.replace(hour=23, minute=59, second=59)
        )
        member3 = self.factory.makePerson()
        self.addToTeamSetExpiryDate(member3, team, datetime3)
    ...
    def test_getExpiringMembershipsToWarn(self):
        ...
        memberships = [
            self.addToTeamSetExpiryDate(
                self.factory.makePerson(), team, now + timedelta(days=2)
            ),
            self.addToTeamSetExpiryDate(
                self.factory.makePerson(), team, now + timedelta(days=8)
            ),
            self.addToTeamSetExpiryDate(
                self.factory.makePerson(), team, now + timedelta(days=15)
            ),
            self.addToTeamSetExpiryDate(
...
class TestTeamParticipationHierarchy(TeamParticipationTestCase):
    ...
    def testRemovingLeafTeam(self):
        ...
        self.assertParticipantsEquals(["name16", "team3", "team4"], self.team2)
...
class TestTeamParticipationTree(TeamParticipationTestCase):
    ...
    def testRemoveTeam3FromTeam2(self):
        ...
        self.assertParticipantsEquals(["name16", "no-priv"], self.team5)
        self.assertEqual(previous_count - 4, self.getTeamParticipationCount())
...
class TestTeamParticipationMesh(TeamParticipationTestCase):
    ...
    def testRemoveTeam3FromTeam2(self):
        ...
        self.assertEqual(previous_count - 3, self.getTeamParticipationCount())
    ...
    def testRemoveTeam5FromTeam4(self):
        ...
        self.assertEqual(previous_count - 10, self.getTeamParticipationCount())
    ...
    def testTeam3_deactivateActiveMemberships(self):
        ...
        self.assertParticipantsEquals([], self.team3)
        self.assertParticipantsEquals(
            ["name16", "no-priv", "team5"], self.team4
        )
        self.assertParticipantsEquals(["name16", "no-priv"], self.team5)
...
class TestTeamMembership(TestCaseWithFactory):
    ...
    def test_teams_not_kicked_from_themselves_bug_248498(self):
        ...
        self.assertTrue(
            teamA.hasParticipationEntryFor(teamA),
            "teamA is not a participant of itself",
        )
        self.assertTrue(
            teamB.hasParticipationEntryFor(teamB),
...
class TestTeamMembershipSetStatus(TestCaseWithFactory):
    ...
    def test_reviewer_is_stored(self):
        ...
        for status, new_statuses in transitions_mapping.items():
            for new_status in new_statuses:
                ...
                self.assertFalse(
                    tm.reviewed_by, "There can be no approver at this point."
                )
                self.assertFalse(
                    tm.date_reviewed,
    ...
    def test_declined_member_can_be_invited(self):
        ...
        tm.setStatus(TeamMembershipStatus.DECLINED, self.team1.teamowner)
        tm.setStatus(TeamMembershipStatus.INVITED, self.team1.teamowner)
    ...
    def test_add_approved(self):
        ...
        self.team1.addMember(member_team, self.team1.teamowner)
        ...
        self.team1.addMember(member_team, member_team.teamowner)
        self.assertEqual(TeamMembershipStatus.APPROVED, tm.status)
...
class TestTeamMembershipJobs(TestCaseWithFactory):
    ...
    def _make_subscribed_bug(
        self, grantee, target, information_type=InformationType.USERDATA
    ):
    ...
    def test_retract_unsubscribes_former_member(self):
        ...
        bug1, ignored = self._make_subscribed_bug(
            person_grantee, product, information_type=InformationType.USERDATA
        )
...
class TestTeamMembershipSendExpirationWarningEmail(TestCaseWithFactory):
    ...
    def runMailJobs(self):
        with dbuser("person-transfer-job"):
            JobRunner.fromReady(
                getUtility(IExpiringMembershipNotificationJobSource)
            ).runAll()
