...
class UsageEnumsMixin:
    ...
    def test_answers_usage_no_data(self):
        ...
        self.assertEqual(ServiceUsage.UNKNOWN, self.target.answers_usage)
    ...
    def test_answers_usage_using_bool(self):
        ...
        self.assertEqual(ServiceUsage.LAUNCHPAD, self.target.answers_usage)
    ...
    def test_answers_usage_with_enum_data(self):
        ...
        self.assertEqual(ServiceUsage.EXTERNAL, self.target.answers_usage)

    def test_answers_setter(self):
        login_person(self.target.owner)
        self.target.official_answers = True
        self.target.answers_usage = ServiceUsage.EXTERNAL
        self.assertEqual(False, self.target.official_answers)
        self.target.answers_usage = ServiceUsage.LAUNCHPAD
        self.assertEqual(True, self.target.official_answers)

    def test_translations_usage_no_data(self):
        # By default, we don't know anything about a target
        self.assertEqual(ServiceUsage.UNKNOWN, self.target.translations_usage)
    ...
    def test_translations_usage_with_enum_data(self):
        ...
        self.assertEqual(ServiceUsage.EXTERNAL, self.target.translations_usage)
    ...
    def test_bug_tracking_usage(self):
        ...
        self.assertEqual(ServiceUsage.UNKNOWN, self.target.bug_tracking_usage)

    def test_blueprints_usage_no_data(self):
        # By default, we don't know anything about a target
        self.assertEqual(ServiceUsage.UNKNOWN, self.target.blueprints_usage)
    ...
    def test_blueprints_usage_using_bool(self):
        ...
        self.assertEqual(ServiceUsage.LAUNCHPAD, self.target.blueprints_usage)
    ...
    def test_blueprints_usage_with_enum_data(self):
        ...
        self.assertEqual(ServiceUsage.EXTERNAL, self.target.blueprints_usage)

    def test_blueprints_setter(self):
        login_person(self.target.owner)
        self.target.official_blueprints = True
        self.target.blueprints_usage = ServiceUsage.EXTERNAL
        self.assertEqual(False, self.target.official_blueprints)
        self.target.blueprints_usage = ServiceUsage.LAUNCHPAD
        self.assertEqual(True, self.target.official_blueprints)
...
class SeriesUsageEnumsMixin:
    ...
    def test_translations_usage_pillar(self):
        ...
        self.assertEqual(
            ServiceUsage.UNKNOWN, self.series_pillar.translations_usage
        )
        self.assertEqual(ServiceUsage.UNKNOWN, self.series.translations_usage)

        self._addCurrentPOTemplate()
        self.assertEqual(
            ServiceUsage.LAUNCHPAD, self.series.translations_usage
        )
...
class TestDistributionUsageEnums(TestCaseWithFactory, UsageEnumsMixin):
    ...
    def test_codehosting_usage(self):
        ...
        self.assertEqual(
            ServiceUsage.NOT_APPLICABLE, self.target.codehosting_usage
        )
...
class TestProductUsageEnums(TestCaseWithFactory, UsageEnumsMixin):
    ...
    def test_codehosting_unknown(self):
        ...
        self.assertEqual(ServiceUsage.UNKNOWN, self.target.codehosting_usage)
    ...
    def test_codehosting_mirrored_branch(self):
        ...
        self.target.development_focus.branch = self.factory.makeProductBranch(
            product=self.target, branch_type=BranchType.MIRRORED
        )
        self.assertEqual(ServiceUsage.EXTERNAL, self.target.codehosting_usage)

    def test_codehosting_remote_branch(self):
        # A remote branch is EXTERNAL.
        login_person(self.target.owner)
        self.target.development_focus.branch = self.factory.makeProductBranch(
            product=self.target, branch_type=BranchType.REMOTE
        )
        self.assertEqual(ServiceUsage.EXTERNAL, self.target.codehosting_usage)
    ...
    def test_codehosting_hosted_branch(self):
        ...
        login_person(self.target.owner)
        self.target.development_focus.branch = self.factory.makeProductBranch(
            product=self.target, branch_type=BranchType.HOSTED
        )
        self.assertEqual(ServiceUsage.LAUNCHPAD, self.target.codehosting_usage)
...
class TestDistroSeriesUsageEnums(TestCaseWithFactory, SeriesUsageEnumsMixin):
    ...
    def _addCurrentPOTemplate(self):
        ...
        self.factory.makeSourcePackage(
            sourcepackagename=sp_name, distroseries=self.series
        )
        self.factory.makePOTemplate(
            distroseries=self.series, sourcepackagename=sp_name
        )
