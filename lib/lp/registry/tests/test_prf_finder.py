...
from lp.testing import TestCase, TestCaseWithFactory, reset_logging
...
class FindReleasesTestCase(TestCase):
    def test_findReleases(self):
        ...
        self.assertEqual(
            prf.seen_products[0], ("product1", ["filter1", "filter2"])
        )
        self.assertEqual(
            prf.seen_products[1], ("product2", ["filter3", "filter4"])
        )
...
class HandleProductTestCase(TestCase):
    ...
    def test_handleProduct(self):
        ...
        class FakeProductReleaseFinder(ProductReleaseFinder):
            ...
            def handleRelease(self, product_name, series_name, url, file_name):
                self.seen_releases.append(
                    (product_name, series_name, os.path.basename(url))
                )
            ...
            for release in ["0", "1"]:
                fp = open(
                    os.path.join(
                        ...
                        "product",
                        series,
                        "product-%s.%s.tar.gz" % (series, release),
                    ),
                    "w",
                )
                fp.write("foo")
        ...
        filters = [
            FilterPattern(
                "series1", self.release_url + "/product/1/product-1.*.tar.gz"
            ),
            FilterPattern(
                "series2", self.release_url + "/product/2/product-2.*.tar.gz"
            ),
        ]
        ...
        self.assertEqual(
            prf.seen_releases[0], ("product", "series1", "product-1.0.tar.gz")
        )
        self.assertEqual(
            prf.seen_releases[1], ("product", "series1", "product-1.1.tar.gz")
        )
        self.assertEqual(
            prf.seen_releases[2], ("product", "series2", "product-2.0.tar.gz")
        )
        self.assertEqual(
            prf.seen_releases[3], ("product", "series2", "product-2.1.tar.gz")
        )
...
class HandleReleaseTestCase(TestCase):
    ...
    def test_handleRelease_alternate_version(self):
        ...
        release_filenames = [
            file_info.libraryfile.filename for file_info in release.files
        ]
    ...
    def test_handleReleaseUnableToParseVersion(self):
        ...
        fp = open(
            os.path.join(self.release_root, "evolution-42.0.tar.gz"), "w"
        )
        fp.write("foo")
        fp.close()
