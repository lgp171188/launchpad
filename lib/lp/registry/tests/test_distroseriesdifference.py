...
class DistroSeriesDifferenceTestCase(TestCaseWithFactory):
    ...
    def test_new_non_derived_series(self):
        ...
        self.assertRaises(
            NotADerivedSeriesError,
            distroseriesdifference_factory.new,
            distro_series,
            source_package_name,
    ...
    def test_source_pub(self):
        ...
        self.assertEqual("foonew", ds_diff.source_pub.source_package_name)
        self.assertEqual(
    ...
    def test_update_resolves_difference(self):
        ...
        self.assertEqual(DistroSeriesDifferenceStatus.RESOLVED, ds_diff.status)
    ...
    def test_update_nulls_diffs_for_resolved(self):
        ...
        was_updated = ds_diff.update()
        self.assertTrue(was_updated)
        self.assertEqual(ds_diff.status, DistroSeriesDifferenceStatus.RESOLVED)
    ...
    def test_latest_comment(self):
        ...
        with person_logged_in(ds_diff.owner):
            comments = [
                ds_diff.addComment(ds_diff.owner, "Wait until version 2.1"),
                ds_diff.addComment(ds_diff.owner, "Wait until version 2.1"),
            ]
    ...
    def test_parent_packagesets(self):
        ...
        ds_diff = self.factory.makeDistroSeriesDifference()
        packagesets = self._setupPackageSets(ds_diff, ds_diff.parent_series, 5)
    ...
    def assertDSDComment(self, ds_diff, dsd_comment, comment_string):
        self.assertEqual(dsd_comment, ds_diff.latest_comment)
        self.assertEqual(
            comment_string, ds_diff.latest_comment.message.text_contents
        )
    ...
    def test_unblocklist_creates_comment(self):
        ...
        with person_logged_in(admin):
            dsd_comment = ds_diff.unblocklist(admin, "Ok now")
        new_status = DistroSeriesDifferenceStatus.NEEDS_ATTENTION
    ...
    def test_cached_properties(self):
        ...
        self.assertContentEqual(["source_pub", "parent_source_pub"], cache)
    ...
    def _setupDSDsWithChangelog(
        self, derived_versions, parent_versions, status=None
    ):
        ...
        parent_changelog = self.factory.makeChangelog(versions=parent_versions)
        transaction.commit()  # Yay, librarian.
        ds_diff = self.factory.makeDistroSeriesDifference(
            status=status,
            versions={
    ...
    def test_requestPackageDiffs(self):
        ...
        ds_diff = self._setupDSDsWithChangelog(["1.0", "1.2"], ["1.0", "1.3"])
        person = self.factory.makePerson()
        with person_logged_in(person):
            ds_diff.requestPackageDiffs(person)

        self.assertEqual("1.2", ds_diff.package_diff.to_source.version)
        self.assertEqual("1.3", ds_diff.parent_package_diff.to_source.version)
        self.assertEqual("1.0", ds_diff.package_diff.from_source.version)
        self.assertEqual(
    ...
    def test_requestPackageDiffs_child_is_base(self):
        ...
        ds_diff = self._setupDSDsWithChangelog(["0.1-1"], ["0.1-1", "0.1-2"])
        person = self.factory.makePerson()
        with person_logged_in(person):
            ds_diff.requestPackageDiffs(person)
    ...
    def test_requestPackageDiffs_parent_is_base(self):
        ...
        ds_diff = self._setupDSDsWithChangelog(["0.1-1", "0.1-2"], ["0.1-1"])
        person = self.factory.makePerson()
        with person_logged_in(person):
            ds_diff.requestPackageDiffs(person)
    ...
    def test_requestPackageDiffs_with_resolved_DSD(self):
        ...
        ds_diff = self._setupDSDsWithChangelog(
            ["0.1-1"], ["0.1-1"], status=DistroSeriesDifferenceStatus.RESOLVED
        )
        person = self.factory.makePerson()
        with person_logged_in(person):
    ...
    def test_source_package_release_pending(self):
        ...
        self.assertEqual("1.5", ds_diff.parent_source_package_release.version)
    ...
    def createPublication(
        ...
        transaction.commit()  # Yay, librarian.
        spr = self.factory.makeSourcePackageRelease(
            sourcepackagename=spn, version=versions[0], changelog=changelog_lfa
        )
    ...
    def test_existing_packagediff_is_linked_when_dsd_created(self):
        ...
        self.createPublication(spn, ["1.2-1", "1.0-1"], dsp.parent_series)
    ...
    def test_multiple_pending_publications_derived(self):
        ...
        ds_diff, _, pub = self._initDiffWithMultiplePendingPublications(
            versions={"derived": "1.0"}, parent=False
        )
        self.assertEqual(pub, ds_diff.source_package_release.publishings[0])

    def test_multiple_pending_publications_parent(self):
        # If multiple (PENDING) publications are present in the parent
        # series, the most recent is returned.
        ds_diff, _, pub = self._initDiffWithMultiplePendingPublications(
            versions={"parent": "1.0"}, parent=True
        )
        self.assertEqual(
            pub, ds_diff.parent_source_package_release.publishings[0]
        )
...
class DistroSeriesDifferenceSourceTestCase(TestCaseWithFactory):
    ...
    def test_getForDistroSeries_filters_by_type(self):
        ...
        self.assertContentEqual(
            [dsds[wanted_type]],
            dsd_source.getForDistroSeries(series, difference_type=wanted_type),
        )
    ...
    def test_getForDistroSeries_matches_by_package_name(self):
        ...
        self.assertContentEqual(
            [dsd], source.getForDistroSeries(series, name_filter=package_name)
        )
    ...
    def test_getForDistroSeries_ignores_parent_packagesets(self):
        ...
        series = dsd.derived_series
        packageset = self.factory.makePackageset(
            distroseries=dsd.parent_series, packages=[dsd.source_package_name]
        )
        packageset_name = packageset.name
        source = getUtility(IDistroSeriesDifferenceSource)
        self.assertContentEqual(
    ...
    def test_getForDistroSeries_sorted_by_package_name(self):
        ...
        results = getUtility(IDistroSeriesDifferenceSource).getForDistroSeries(
            series
        )
    ...
    def test_getForDistroSeries_filters_by_parent(self):
        ...
        dsds = [
            self.factory.makeDistroSeriesDifference(
                parent_series=dsp.parent_series, derived_series=derived_series
            )
            for dsp in dsps
        ]
        dsd_source = getUtility(IDistroSeriesDifferenceSource)
...
class TestMostRecentComments(TestCaseWithFactory):
    ...
    def test_most_recent_comments(self):
        ...
        self.assertContentEqual(expected_comments, most_recent_comments(dsds))
...
class TestMostRecentPublications(TestCaseWithFactory):
    ...
    def test_simple(self):
        ...
        source_pubs_by_spn_id_expected = {
            (dsd.source_package_name.id, dsd.source_pub) for dsd in dsds
        }
        ...
        self.assertContentEqual(
            source_pubs_by_spn_id_expected, source_pubs_by_spn_id_found
        )
        # Parent publication
        parent_source_pubs_by_spn_id_expected = {
            (dsd.source_package_name.id, dsd.parent_source_pub) for dsd in dsds
        }
    ...
    def test_statuses(self):
        ...
        self.assertContentEqual(
            ...
            most_recent_publications(
                [dsd],
                in_parent=False,
                statuses=(PackagePublishingStatus.DELETED,),
            ),
        )
