...
class TestDeriveDistroSeries(TestCaseWithFactory):
    ...
    def test_init_creates_new_job(self):
        self.child.initDerivedDistroSeries(self.child.driver, [self.parent.id])
        [job] = list(getUtility(IInitializeDistroSeriesJobSource).iterReady())
