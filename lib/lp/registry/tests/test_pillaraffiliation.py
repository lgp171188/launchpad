...
class TestPillarAffiliation(TestCaseWithFactory):
    ...
    def _check_affiliated_with_distro(self, person, distro, role):
        [badges] = IHasAffiliation(distro).getAffiliationBadges([person])
        self.assertEqual(("/@@/distribution-badge", "Pting", role), badges[0])
    ...
    def test_distro_affiliation_multiple_people(self):
        ...
        distro = self.factory.makeDistribution(
            owner=people[0], driver=people[1], name="pting"
        )
...
class _TestBugTaskorBranchMixin:
    ...
    def test_product_bug_supervisor_affiliation(self):
        # A person who is the bug supervisor for a distro is affiliated.
        person = self.factory.makePerson()
        product = self.factory.makeProduct(bug_supervisor=person, name="pting")
...
class TestBugTaskPillarAffiliation(
    ...
    def test_affiliated_with_multiple_bugtasks(self):
        ...
        [badges] = IHasAffiliation(bug.default_bugtask).getAffiliationBadges(
            [person]
        )
...
class TestProductSeriesPillarAffiliation(TestCaseWithFactory):
    ...
    def test_driver_affiliation(self):
        ...
        [badges] = IHasAffiliation(productseries).getAffiliationBadges(
            [driver]
        )
        self.assertEqual(("/@@/product-badge", "Pting", "driver"), badges[0])
...
class TestQuestionPillarAffiliation(TestCaseWithFactory):
    ...
    def test_answer_contact_affiliation_for_distro(self):
        ...
        [badges] = IHasAffiliation(question).getAffiliationBadges(
            [answer_contact]
        )
        self.assertEqual(
        ...
        self.assertEqual(
            ("/@@/distribution-badge", distro.displayname, "driver"), badges[1]
        )
        self.assertEqual(
            ("/@@/distribution-badge", distro.displayname, "answer contact"),
            badges[2],
        )
    ...
    def test_answer_contact_affiliation_for_distro_sourcepackage(self):
        ...
        self.assertEqual(
            ("/@@/distribution-badge", distribution.displayname, "maintainer"),
            badges[0],
        )
        self.assertEqual(
            ("/@@/distribution-badge", distribution.displayname, "driver"),
            badges[1],
        )
        self.assertEqual(
            (
                "/@@/distribution-badge",
                distro_sourcepackage.displayname,
                "answer contact",
            ),
            badges[2],
        )
    ...
    def test_product_affiliation(self):
        ...
        self.assertEqual(
            ("/@@/product-badge", product.displayname, "maintainer"), badges[0]
        )
...
class TestSpecificationPillarAffiliation(TestCaseWithFactory):
    ...
    def test_product_affiliation(self):
        ...
        product = self.factory.makeProduct(owner=person)
        specification = self.factory.makeSpecification(product=product)
        [badges] = IHasAffiliation(specification).getAffiliationBadges(
            [person]
        )
        self.assertEqual(
            ("/@@/product-badge", product.displayname, "maintainer"), badges[0]
