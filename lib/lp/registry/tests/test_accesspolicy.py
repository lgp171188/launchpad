...
from lp.testing import TestCaseWithFactory, record_two_runs
...
def get_policies_for_artifact(concrete_artifact):
    ...
    return [
        apa.policy
        for apa in getUtility(IAccessPolicyArtifactSource).findByArtifact(
            [artifact]
        )
    ]
...
class TestAccessPolicySource(TestCaseWithFactory):
    ...
    def test_create(self):
        ...
        self.assertContentEqual(
            wanted, [(policy.pillar, policy.type) for policy in policies]
        )
    ...
    def test_find(self):
        ...
        self.assertContentEqual(
            query,
            [
                (policy.pillar, policy.type)
                for policy in getUtility(IAccessPolicySource).find(query)
            ],
        )
    ...
    def test_delete(self):
        ...
        self.assertContentEqual(policies[:2], ap_source.findByPillar(pillars))
...
class TestAccessArtifact(TestCaseWithFactory):
    ...
    def test_provides_interface(self):
        self.assertThat(
            self.factory.makeAccessArtifact(), Provides(IAccessArtifact)
        )
...
class BaseAccessArtifactTests:
    ...
    def test_ensure(self):
        ...
        self.assertContentEqual(
            concretes, [abstract.concrete_artifact for abstract in abstracts]
        )
...
class TestAccessArtifactBranch(BaseAccessArtifactTests, TestCaseWithFactory):
    def getConcreteArtifact(self):
        return self.factory.makeBranch()


class TestAccessArtifactGitRepository(
    BaseAccessArtifactTests, TestCaseWithFactory
):
    def getConcreteArtifact(self):
        return self.factory.makeGitRepository()


class TestAccessArtifactBug(BaseAccessArtifactTests, TestCaseWithFactory):
    def getConcreteArtifact(self):
        return self.factory.makeBug()


class TestAccessArtifactSpecification(
    BaseAccessArtifactTests, TestCaseWithFactory
):
    def getConcreteArtifact(self):
...
class TestAccessPolicyArtifactSource(TestCaseWithFactory):
    ...
    def test_create(self):
        ...
        self.assertContentEqual(
            wanted, [(link.abstract_artifact, link.policy) for link in links]
        )
...
class TestAccessPolicyGrant(TestCaseWithFactory):
    ...
    def test_provides_interface(self):
        self.assertThat(
            self.factory.makeAccessPolicyGrant(), Provides(IAccessPolicyGrant)
        )
...
class TestAccessPolicyGrantSource(TestCaseWithFactory):
    ...
    def test_findByPolicy(self):
        ...
        policy = self.factory.makeAccessPolicy()
        grants = [
            self.factory.makeAccessPolicyGrant(policy=policy) for i in range(3)
        ]
        self.factory.makeAccessPolicyGrant()
        self.assertContentEqual(
            grants, getUtility(IAccessPolicyGrantSource).findByPolicy([policy])
        )
    ...
    def test_revoke(self):
        ...
        other_grants.extend(
            [self.factory.makeAccessPolicyGrant() for i in range(3)]
        )
