...
class SharingJobDerivedTestCase(TestCaseWithFactory):
    ...
    def test_repr_branches(self):
        ...
        self.assertEqual(
            "<REMOVE_ARTIFACT_SUBSCRIPTIONS job reconciling subscriptions "
            "for branch_ids=[%d], requestor=%s>" % (branch.id, requestor.name),
            repr(job),
        )
    ...
    def test_repr_gitrepositories(self):
        ...
        self.assertEqual(
            ...
            % (gitrepository.id, requestor.name),
            repr(job),
        )
    ...
    def test_repr_snaps(self):
        ...
        self.assertEqual(
            "<REMOVE_ARTIFACT_SUBSCRIPTIONS job reconciling subscriptions "
            "for requestor=%s, snap_ids=[%d]>" % (requestor.name, snap.id),
            repr(job),
        )
    ...
    def test_repr_specifications(self):
        ...
        self.assertEqual(
            ...
            % (requestor.name, specification.id),
            repr(job),
        )
...
class RemoveArtifactSubscriptionsJobTestCase(TestCaseWithFactory):
    ...
    def _assert_artifact_change_unsubscribes(
        self, change_callback, configure_test
    ):
    ...
    def test_unsubscribe_pillar_artifacts_specific_info_types(self):
        ...
        bug1, ignored = self._make_subscribed_bug(
            person_grantee, pillar, information_type=InformationType.USERDATA
        )
