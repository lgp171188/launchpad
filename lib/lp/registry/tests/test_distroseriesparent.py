...
class TestDistroSeriesParent(TestCaseWithFactory):
    ...
    def test_getByDerivedSeries(self):
        parent_series = self.factory.makeDistroSeries()
        derived_series = self.factory.makeDistroSeries()
        self.factory.makeDistroSeriesParent(derived_series, parent_series)
...
class TestDistroSeriesParentSecurity(TestCaseWithFactory):
    ...
    def test_random_person_is_unauthorized(self):
        ...
        with person_logged_in(person):
            self.assertRaises(
                Unauthorized, setattr, dsp, "derived_series", dsp.parent_series
            )
...
class TestOverlayTree(TestCaseWithFactory):
    ...
    def test_getFlattenedOverlayTree(self):
        ...
        dsp_parent11_parent12 = self.factory.makeDistroSeriesParent(
            derived_series=parent11,
            parent_series=parent12,
            initialized=True,
            is_overlay=True,
            pocket=PackagePublishingPocket.RELEASE,
            component=universe_component,
        ...
        # parent12 -> parent13
        self.factory.makeDistroSeriesParent(
            derived_series=parent12, initialized=True, is_overlay=False
        )
        # series -> parent21
        dsp_series_parent21 = self.factory.makeDistroSeriesParent(
            derived_series=distroseries,
            parent_series=parent21,
            initialized=True,
            is_overlay=True,
            pocket=PackagePublishingPocket.RELEASE,
            component=universe_component,
        ...
        # series -> parent31
        self.factory.makeDistroSeriesParent(
            derived_series=distroseries, initialized=True, is_overlay=False
        )
        # series -> parent41
        ...
        overlays = getUtility(IDistroSeriesParentSet).getFlattenedOverlayTree(
            distroseries
        )
        ...
        self.assertContentEqual(
            [
                dsp_series_parent11,
                dsp_parent11_parent12,
                dsp_series_parent21,
                dsp_parent21_parent22,
                dsp_series_parent41,
            ],
            overlays,
        )
