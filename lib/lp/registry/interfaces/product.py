...
class LicenseStatus(DBEnumeratedType):
    ...
    OPEN_SOURCE = DBItem(
        10, "Open Source", "This project&rsquo;s licence is open source."
    )
    PROPRIETARY = DBItem(
        20, "Proprietary", "This project&rsquo;s licence is proprietary."
    )
    UNREVIEWED = DBItem(
        30, "Unreviewed", "This project&rsquo;s licence has not been reviewed."
    )
    UNSPECIFIED = DBItem(
        40,
        "Unspecified",
        "This project&rsquo;s licence has not been specified.",
    )
...
class License(DBEnumeratedType):
    ...
    EDUCATIONAL_COMMUNITY = DBItem(
        100,
        "Educational Community Licence",
        url="http://www.opensource.org/licenses/ecl2.php",
    )
    GNU_GPL_V2 = DBItem(
        130, "GNU GPL v2", url="http://www.opensource.org/licenses/gpl-2.0.php"
    )
    GNU_GPL_V3 = DBItem(
        135,
        "GNU GPL v3",
    ...
    PERL = DBItem(200, "Perl Licence")
    PHP = DBItem(
        210, "PHP Licence", url="http://www.opensource.org/licenses/php.php"
    )
    ...
    GNU_FDL_NO_OPTIONS = DBItem(
        330, "GNU FDL no options", url="http://www.gnu.org/copyleft/fdl.html"
    )
    OFL = DBItem(
        340, "Open Font Licence v1.1", url="http://scripts.sil.org/OFL"
    )
...
class IProductModerateRestricted(Interface):
    ...
    project_reviewed = exported(
        Bool(
            ...
            description=_(
                "Whether or not this project has been reviewed. "
...
class IProductPublic(Interface):
    ...
    private = exported(
        Bool(
            title=_("Product is confidential"),
            required=False,
            readonly=True,
            default=False,
            description=_(
...
class IProductLimitedView(IHasIcon, IHasLogo, IHasOwner, ILaunchpadUsage):
    ...
    def getOCIProject(name):
        """Return an `OCIProject` with this name for this product, or None."""
...
class IProductView(
    ...
    autoupdate = Bool(
        ...
        description=_(
            "Whether or not this project's attributes are "
    ...
    private_bugs = exported(
        Bool(
            title=_("Private bugs (obsolete; always False)"),
            readonly=True,
    ...
    licenses = exported(
        Set(title=_("Licences"), value_type=Choice(vocabulary=License))
    )
    ...
    bugtracker = exported(
        ProductBugTracker(
            title=_("Bugs are tracked"), vocabulary="BugTracker"
        ),
    ...
    distrosourcepackages = Attribute(
        _("List of distribution packages for " "this product")
    )
    ...
    translation_focus = exported(
        ReferenceChoice(
            ...
            vocabulary="FilteredProductSeries",
            schema=IProductSeries,
            description=_("Project series that translators should focus on."),
        )
    )

    translatable_packages = Attribute(
        "A list of the source packages for this product that can be "
        "translated sorted by distroseries.version (descending order) "
    ...
    translationgroups = Attribute(
        "The list of applicable translation "
    ...
    remote_product = exported(
        TextLine(
            title=_("Remote bug tracker project id"),
            required=False,
            description=_(
    ...
    vcs = exported(
        Choice(
            ...
            required=False,
            vocabulary=VCSType,
            description=_("Version control system for this project's code."),
        )
    )
    ...
    @operation_parameters(version=TextLine(title=_("Version"), required=True))
...
class IProductEditRestricted(IOfficialBugTagTargetRestricted, IWebhookTarget):
    ...
    @operation_parameters(
        bug_sharing_policy=copy_field(IProductView["bug_sharing_policy"])
    )
    @export_write_operation()
    ...
    @operation_parameters(
        branch_sharing_policy=copy_field(IProductView["branch_sharing_policy"])
    )
    @export_write_operation()
    @operation_for_version("devel")
...
class IProductSet(Interface):
    ...
    @rename_parameters_as(
        ...
        screenshotsurl="screenshots_url",
        freshmeatproject="freshmeat_project",
        wikiurl="wiki_url",
    ...
    @operation_parameters(
        ...
        licenses=Set(
            title=_("Licenses"), value_type=Choice(vocabulary=License)
        ),
        ...
        subscription_expires_after=Date(title=_("Subscription expires after")),
...
class IProductReviewSearch(Interface):
    ...
    search_text = TextLine(
        title=_("Search text"),
        description=_(
            "Search text in the product's name, displayname, title, "
    ...
    active = Choice(
        title=_("Active"), values=[True, False], required=False, default=True
    )
    ...
    subscription_expires_before = Date(title=_("and"), required=False)

    subscription_modified_after = Date(
        title=_("Subscription modified between"), required=False
    )

    subscription_modified_before = Date(title=_("and"), required=False)
