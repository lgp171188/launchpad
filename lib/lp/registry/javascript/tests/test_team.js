    ...
    tests.suite.add(new Y.Test.Case({
        ...
        setUp: function () {
            ...
            function(url, func, config) {
                config.on.success([false, 'administrator']);
            };
            window.LP = {
                "cache": {
                    "related_features": {},
        ...
        // When a user is already a member of a team and it uses
        // "Pick me" button an error message is displayed
        test_pick_me_error_message: function() {
            namespace.setup_add_member_handler('Add member handler');
            var add_member_link = Y.one(".menu-link-add_member");
            add_member_link.simulate('click');
            var assign_me_button = Y.one(".yui-picker-assign-me-button");
            assign_me_button.simulate('click');
            var childNodes = Y.one('.yui3-widget-bd')._node.childNodes;
            Y.Assert.isTrue(childNodes.length > 0);
            var widget_box_text = childNodes[0].innerText;
            Y.Assert.areEqual(
                "user1 is already administrator as a member of the team.\nOK",
                widget_box_text
            );
        },
