...
from testtools.matchers import Equals, KeysEqual
...
class TestExposeAdministeredTeams(TestCaseWithFactory):
    ...
    def _setup_teams(self, owner):
        ...
        self.factory.makeTeam(name="unrelated-team", owner=owner)
    ...
    def test_teams_preferredemail(self):
        ...
        self.factory.makeEmail(
            "foo@example.net", team2, email_status=EmailAddressStatus.PREFERRED
        )

        expose_user_administered_teams_to_js(
            self.request, self.user, context, absoluteURL=fake_absoluteURL
        )
...
class TestIntegrationExposeUserSubscriptionsToJS(TestCaseWithFactory):
    ...
    def test_team_admin_subscription(self):
        ...
        with person_logged_in(team.teamowner):
            team.addMember(
                user, team.teamowner, status=TeamMembershipStatus.ADMIN
            )
