...
from lp.layers import FeedsLayer, setFirstLayer
...
class TestBugTaskView(TestCaseWithFactory):
    ...
    def test_rendered_query_counts_constant_with_team_memberships(self):
        ...
        recorder1, recorder2 = record_two_runs(
            ...
            0,
            10,
            login_method=lambda: login(ADMIN_EMAIL),
        )
    ...
    def test_rendered_query_counts_reduced_with_branches(self):
        ...
        recorder1, recorder2 = record_two_runs(
            lambda: self.getUserBrowser(url, owner), make_merge_proposals, 0, 1
        )
    ...
    def test_changing_milestone_and_assignee_with_lifecycle(self):
        ...
        with person_logged_in(product.owner):
            form_data = {
                ...
                "%s.assignee.option"
                % product.name: "%s.assignee.assign_to_me"
                % product.name,
    ...
    def test_bugtag_urls_are_encoded(self):
        ...
        expected = [
            ("depends-on+987", "/foobar/+bugs?field.tag=depends-on%2B987")
        ]
        self.assertEqual(expected, view.unofficial_tags)
        browser = self.getUserBrowser(canonical_url(bug), bug.owner)
        self.assertIn(
            'href="/foobar/+bugs?field.tag=depends-on%2B987"', browser.contents
        )
...
class TestBugTasksNominationsView(TestCaseWithFactory):
    ...
    def makeDuplicate(self):
        ...
        self.assertEqual(2, self.bug2.users_affected_count)
    ...
    def test_counts_user_unaffected(self):
        self.useFixture(
            FeatureFixture({"bugs.affected_count_includes_dupes.disabled": ""})
        )
        self.makeDuplicate()
    ...
    def test_bugtarget_parent_shown_for_orphaned_series_tasks(self):
        ...
        self.assertEqual(
            [
                ...
                (
                    bug.getBugTask(series),
                    "+bugtasks-and-nominations-table-row",
                ),
                (
                    bug.getBugTask(new_prod),
                    "+bugtasks-and-nominations-table-row",
                ),
            ],
        ...
        self.assertIn(
            'href="%s"'
            % canonical_url(series.product, path_only_if_possible=True),
            content,
        )
...
class TestBugTaskDeleteView(TestCaseWithFactory):
    ...
    def test_delete_view_rendering(self):
        ...
        server_url = canonical_url(getUtility(ILaunchpadRoot), rootsite="bugs")
        ...
        confirmation_message = find_tag_by_id(contents, "confirmation-message")
    ...
    def test_ajax_delete_non_current_bugtask(self):
        ...
        self.assertIn(bug.default_bugtask.bugtargetdisplayname, target_link)
...
class TestBugTaskEditViewStatusField(TestCaseWithFactory):
    ...
    def assertStatuses(self, titles, bugtask, user):
        ...
        js_statuses = [
            item["value"]
            for item in bugtask_data["status_widget_items"]
...
class TestBugTaskEditViewAssigneeField(TestCaseWithFactory):
    ...
    def test_assignee_vocabulary_regular_user_without_bug_supervisor(self):
        ...
        view.initialize()
        self.assertEqual(
            "ValidAssignee", view.form_fields["assignee"].field.vocabularyName
        )
...
class TestBugTaskEditView(WithScenarios, TestCaseWithFactory):
    ...
    def test_retarget_already_exists_error(self):
        ...
        bug_task_2 = self.factory.makeBugTask(bug=bug_task_1.bug, target=dsp_2)
        form = {
    ...
    def test_retarget_product_with_milestone(self):
        ...
        view = create_initialized_view(bug_task, name="+editstatus", form=form)
    ...
    def test_retarget_sourcepackage_to_distroseries(self):
        ...
        view = self.createNameChangingViewForSourcePackageTask(bug_task, "")
        self.assertEqual([], view.errors)
...
class TestDistributionSourcePackageBugs(
    TestCaseWithFactory, BugTaskViewTestMixin
):
...
class TestBugActivityItem(TestCaseWithFactory):
    ...
    def test_change_details_bug_lock_reason_set(self):
        ...
        self.assertEqual(
            "too hot", BugActivityItem(bug.activity.last()).change_details
        )
    ...
    def test_change_details_bug_lock_reason_unset(self):
        ...
        self.assertEqual(
            "Unset", BugActivityItem(bug.activity.last()).change_details
        )

    def test_change_details_bugtask_importance_explanation_set(self):
        with celebrity_logged_in("admin"):
            task = self.factory.makeBugTask()
            self.setAttribute(task, "importance_explanation", "Critical")
        self.assertEqual(
            "unset &#8594; Critical",
            BugActivityItem(task.bug.activity.last()).change_details,
        )
    ...
    def test_change_details_bugtask_status_explanation_set(self):
        with celebrity_logged_in("admin"):
            task = self.factory.makeBugTask()
            self.setAttribute(
                task, "status_explanation", "This is blocked on foo"
            )
    ...
    def test_change_details_bugtask_importance_explanation_unset(self):
        with celebrity_logged_in("admin"):
            task = self.factory.makeBugTask()
            self.setAttribute(task, "importance_explanation", "Critical")
            self.setAttribute(task, "importance_explanation", None)
    ...
    def test_change_details_bugtask_status_explanation_unset(self):
        with celebrity_logged_in("admin"):
            ...
            self.setAttribute(task, "status_explanation", None)
    ...
    def test_change_details_bugtask_status_explanation_changed(self):
        with celebrity_logged_in("admin"):
            ...
            self.setAttribute(
                task, "status_explanation", "This is blocked on bar"
            )
...
class TestBugTaskBatchedCommentsAndActivityView(TestCaseWithFactory):
    ...
    def _makeNoisyBug(
        self, comments_only=False, number_of_comments=10, number_of_changes=10
    ):
        """Create and return a bug with a lot of comments and activity."""
        bug = self.factory.makeBug()
        with person_logged_in(bug.owner):
            if not comments_only:
                for i in range(number_of_changes):
                    change = BugTaskStatusChange(
                        bug.default_bugtask,
                        UTC_NOW,
                        bug.default_bugtask.product.owner,
                        "status",
                        BugTaskStatus.NEW,
    ...
    def _assertThatUnbatchedAndBatchedActivityMatch(
        self, unbatched_activity, batched_activity
    ):
    ...
    def test_event_groups_excludes_visible_recent_comments(self):
        ...
        self.assertEqual(
            expected_length,
            actual_length,
            "Expected %i comments, got %i." % (expected_length, actual_length),
        )
...
def make_bug_task_listing_item(
    ...
    bug_badge_properties = bug_task_set.getBugTaskBadgeProperties([bugtask])
...
class TestBugTaskSearchListingView(BrowserTestCase):
    ...
    def test_rendered_query_counts_constant_with_many_bugtasks(self):
        ...
        recorder1, recorder2 = record_two_runs(
            ...
            1,
            10,
            login_method=lambda: login(ANONYMOUS),
        )
    ...
    def test_mustache_rendering_obfuscation(self):
        ...
        bug_task, browser = self.getBugtaskBrowser(
            title="a@example.com", no_login=True
        )
    ...
    def test_sort_keys_in_json_cache_data(self):
        ...
        for key in json_sort_keys:
            self.assertEqual(3, len(key), "Invalid key length: %r" % (key,))
            self.assertTrue(
                key[2] in ("asc", "desc"), "Invalid order value: %r" % (key,)
            )
...
class TestBugListingBatchNavigator(TestCaseWithFactory):
    ...
    def test_mustache_listings_escaped(self):
        ...
        navigator = BugListingBatchNavigator([], LaunchpadTestRequest(), [], 0)
...
class TestBugTaskListingItem(TestCaseWithFactory):
    ...
    def test_model(self):
        ...
        with person_logged_in(owner):
            ...
            expected = (
                '<span alt="%s" title="%s" class="sprite private">' "</span>"
            ) % (
    ...
    def test_tag_urls_use_view_context(self):
        ...
        self.assertTrue(
            url.startswith(canonical_url(project_group, view_name="+bugs"))
        )
    ...
    def test_urls_without_target_context(self):
        ...
        url = item.model["tags"][0]["url"]
        self.assertTrue(
            url.startswith(canonical_url(product, view_name="+bugs"))
        )
