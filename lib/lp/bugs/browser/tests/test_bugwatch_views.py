...
class TestBugWatchEditView(TestCaseWithFactory):
    ...
    def setUp(self):
        ...
        self.bug_task = self.factory.makeBug(owner=self.person).default_bugtask
        self.bug_watch = self.factory.makeBugWatch(bug=self.bug_task.bug)
    ...
    def test_cannot_delete_watch_if_linked_to_comment(self):
        ...
        message = getUtility(IMessageSet).fromText(
            "Example message",
            "With some example content to read.",
            owner=self.person,
        )
