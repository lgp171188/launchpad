...
from lp.registry.model.milestone import Milestone, milestone_sort_key
from lp.services.database.bulk import load_related
from lp.services.feeds.browser import FeedsMixin
from lp.services.propertycache import cachedproperty
from lp.services.webapp.batching import BatchNavigator
from lp.services.webapp.menu import Link, NavigationMenu
...
def get_package_search_url(dsp_bugs_url, extra_params=None):
    ...
    if extra_params is not None:
        ...
        if "field.searchtext" in extra_params:
            extra_params["field.searchtext"] = extra_params[
                "field.searchtext"
            ].encode("utf8")
...
class PersonBugsMenu(NavigationMenu):
    ...
    def relatedbugs(self):
        ...
        summary = (
            "All bug reports which %s reported, is assigned to, "
    ...
    def subscribedbugs(self):
        ...
        summary = "Bug reports %s is subscribed to." % self.context.displayname
        return Link("+subscribedbugs", text, site="bugs", summary=summary)

    def commentedbugs(self):
        text = "Commented bugs"
        summary = (
            "Bug reports on which %s has commented." % self.context.displayname
        )
        enabled = not self.context.is_team
        return Link(
            "+commentedbugs",
...
class FilteredSearchListingViewMixin(
...
class PersonRelatedBugTaskSearchListingView(
    FilteredSearchListingViewMixin, FeedsMixin
):
...
class PersonSubscriptionsView(LaunchpadView):
    ...
    def canUnsubscribeFromBugTasks(self):
        ...
        return self.user is not None and self.user.inTeam(self.context)
