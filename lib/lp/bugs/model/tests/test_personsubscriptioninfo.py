...
class TestPersonSubscriptionInfo(TestCaseWithFactory):
    ...
    def assertCollectionContents(
        self, collection, personal=0, as_team_member=0, as_team_admin=0
    ):
        # Make sure that the collection has the values we expect.
        self.assertEqual(
            collection.count, personal + as_team_member + as_team_admin
        )
        for name, expected in (
            ...
            if IVirtualSubscriptionInfoCollection.providedBy(collection):
                ...
                self.assertThat(
                    collection, Provides(IRealSubscriptionInfoCollection)
                )
    ...
    def assertRealSubscriptionInfoMatches(
        self, info, bug, principal, principal_is_reporter, bug_supervisor_tasks
    ):
    ...
    def test_direct(self):
        ...
        self.assertFalse(self.subscriptions.muted)
        self.assertCollectionContents(self.subscriptions.direct, personal=1)
    ...
    def test_owner(self):
        ...
        self.assertFalse(self.subscriptions.muted)
        self.assertCollectionContents(self.subscriptions.as_owner, personal=1)
        self.assertVirtualSubscriptionInfoMatches(
