...
class BugNotification(StormBase):
    ...
    def recipients(self):
        ...
        return (
            IStore(BugNotificationRecipient)
            .find(
                BugNotificationRecipient,
...
class BugNotificationSet:
    ...
    def getNotificationsToSend(self):
        ...
        results = list(
            ...
            ).order_by("BugNotification.bug", "-BugNotification.id")
        )
        interval = timedelta(
    ...
    def addNotification(
        self, bug, is_comment, message, recipients, activity, deferred=False
    ):
        """See `IBugNotificationSet`."""
        if deferred:
    ...
    def getRecipientFilterData(self, bug, recipient_to_sources, notifications):
        ...
        if len(source_person_id_map) == 0:
            ...
            filter_data = source.find(
                ...
                In(
                    BugNotificationFilter.bug_notification_id,
        ...
        if filter_ids:
            ...
            mute_data = store.find(
                ...
                In(
                    BugSubscriptionFilterMute.person_id, list(recipient_id_map)
                ),
...
class BugNotificationFilter(StormBase):
    ...
    bug_notification_id = Int("bug_notification", allow_none=False)
    bug_notification = Reference(bug_notification_id, "BugNotification.id")
