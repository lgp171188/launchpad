...
from lp.bugs.model.bugwatch import BugWatchDeletionError, get_bug_watch_ids
    ...
    scenarios = [
        ...
        (
            "CPAN",
            {
                "bugtracker_type": BugTrackerType.RT,
                "bug_url": (
                    "http://rt.cpan.org/Public/Bug/Display.html?id=2379"
                ),
...
class TestBugWatch(TestCaseWithFactory):
    ...
    def test_bugtasks_to_update(self):
        ...
        self.assertEqual(
            [product_task], list(removeSecurityProxy(watch).bugtasks_to_update)
        )
...
class TestBugWatchSet(TestCaseWithFactory):
    ...
    def test_getBugWatchesForRemoteBug(self):
        ...
        self.assertEqual(
            ...
            set(
                bug_watch_set.getBugWatchesForRemoteBug(
                    "bob", [bug_watch.id for bug_watch in bug_watches_limited]
                )
            ),
        )
...
class TestBugWatchSetBulkOperations(TestCaseWithFactory):
    ...
    def _checkStatusOfBugWatches(
        ...
        for bug_watch in self.bug_watches:
            ...
            self.assertEqual(next_check_is_null, bug_watch.next_check is None)
            self.assertEqual(last_error_type, bug_watch.last_error_type)
...
class TestBugWatchBugTasks(TestCaseWithFactory):
    ...
    def test_bugtasks(self):
        ...
        self.assertIsInstance(self.bug_watch.bugtasks, list)
