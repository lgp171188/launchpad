...
def print_bugwatches(bug_watches, convert_remote_status=None):
    ...
    """
    watches = {
        int(bug_watch.remotebug): bug_watch for bug_watch in bug_watches
    }
...
class TestBugzilla(BugTrackerResponsesMixin, Bugzilla):
    ...
    def _getCallback(self, request):
        ...
        if url.path == urlsplit(self.baseurl).path + "/xml.cgi" and parse_qs(
            url.query
        ).get("id") == ["1"]:
...
class TestBugzillaXMLRPCTransport(RequestsTransport):
    ...
    def has_valid_auth_cookie(self):
        return (
            self.auth_cookie is not None
            and self.auth_cookie is not self.expired_cookie
        )
    ...
    def request(self, host, handler, request, verbose=None):
        ...
        assert (
            method_name in self.methods[method_prefix]
        ...
        if (
            method_name in self.auth_required_methods
            and not self.has_valid_auth_cookie
        ):
    ...
    def get_bugs(self, arguments):
        ...
        assert (
            bug_ids is not None or products is not None
        ...
        for id in bug_ids:
            ...
            if (
                ...
                and bug_dict["last_change_time"] < changed_since
            ):
                continue

            # If the bug doesn't belong to one of the products in the
            # products list, ignore it.
            if products is not None and bug_dict["product"] not in products:
                continue
...
class TestBugzillaAPIXMLRPCTransport(TestBugzillaXMLRPCTransport):
        ...
        for user in self.users:
            if user["login"] == login and user["password"] == password:
                ...
                raise xmlrpc.client.Fault(
                    300, "The username or password you entered is not valid."
                )
    ...
    def search(self, arguments):
        ...
        if arguments.get("id") is not None:
            ...
            search_args["ids"] = [bug_id for bug_id in self.bugs]
    ...
    def update_see_also(self, arguments):
        ...
        assert (
            "add" in arguments or "remove" in arguments
        ...
        for bug_id in arguments["ids"]:
            ...
            for url in arguments.get("add", []):
                ...
                if "launchpad" not in url and "show_bug.cgi" not in url:
                    raise xmlrpc.client.Fault(
...
class TestTrac(BugTrackerResponsesMixin, Trac):
    ...
    def addResponses(
        self, requests_mock, supports_single_exports=True, broken=False
    ):
...
class TestTracXMLRPCTransport(RequestsTransport):
    ...
    def request(self, host, handler, request, verbose=None):
        ...
        if self.auth_cookie is None or self.auth_cookie == self.expired_cookie:
            # All the Trac XML-RPC methods need authentication.
            raise xmlrpc.client.ProtocolError(
                method_name, errcode=403, errmsg="Forbidden", headers=None
            )
    ...
    def bug_info(self, level, criteria=None):
        ...
        if "modified_since" in criteria:
            ...
            # to a datetime.
            modified_since = datetime.fromtimestamp(criteria["modified_since"])
        ...
        if "bugs" in criteria:
            bugs_to_return = [
                bug for bug in bugs_to_return if bug.id in criteria["bugs"]
            ]
        ...
        # At level 2, we also return comment IDs for each bug.
            ...
            bugs_to_return = [
                dict(
                    bug.asDict(),
                    comments=[comment["id"] for comment in bug.comments],
                )
                for bug in bugs_to_return
            ]
...
class TestRequestTracker(BugTrackerResponsesMixin, RequestTracker):
    ...
    def _getCallback(self, request):
        ...
        if url.path == "/":
            ...
            if url.path == "/REST/1.0/search/ticket/":
                ...
                bug_id = re.match(
                    r"/REST/1\.0/ticket/([0-9]+)/show", url.path
                ).group(1)
    ...
    def addResponses(self, requests_mock, bad=False):
        ...
        if bad:
            requests_mock.add(
                "GET",
                re.compile(re.escape(self.baseurl)),
...
class TestDebianBug(debbugs.Bug):
    ...
    def __init__(
        self,
        reporter_email="foo@example.com",
        package="evolution",
...
class TestDebBugsDB:
    ...
    def __init__(self):
        self._data_path = os.path.join(os.path.dirname(__file__), "testfiles")
    ...
    def load_log(self, bug):
        ...
        if self.fail_on_load_log:
            raise debbugs.LogParseFailed("debbugs-log.pl exited with code 512")

        with open(self.data_file, "rb") as f:
            comment_data = f.read()
        bug._emails = []
        bug.comments = [
            comment.strip() for comment in comment_data.split(b"--\n")
        ]
