...
from testtools.matchers import AllMatch, MatchesStructure, StartsWith
...
class TestBugChanges(TestCaseWithFactory):
    ...
    def changeAttribute(self, obj, attribute, new_value):
        ...
        with notify_modified(
            obj, [attribute], user=self.user
        ) as obj_before_modification:
    ...
    def assertRecordedChange(
        self, expected_activity=None, expected_notification=None, bug=None
    ):
        ...
        if expected_notification is None:
            ...
            for expected_notification in expected_notifications:
                ...
                expected_recipient_reasons = expected_notification.get(
                    "recipient_reasons"
                )
    ...
    def test_bugwatch_added_from_comment(self):
        ...
        self.bug.newMessage(
            content="http://bugs.example.com/view.php?id=1234", owner=self.user
        )
        ...
        bugwatch_notification = {
            ...
            "person": self.user,
            "recipients": [self.user, self.product_metadata_subscriber],
        }
    ...
    def test_bugwatch_modified(self):
        ...
        bugwatch_removal_notification = {
            "text": (
                "** Bug watch removed: %s #%s\n"
                "   %s" % (bug_watch.bugtracker.title, old_remotebug, old_url)
            ),
            "person": self.user,
        }
    ...
    def test_tags_removed(self):
        ...
        self.bug.tags = ["first-new-tag", "second-new-tag"]
        self.saveOldChanges()
        self.changeAttribute(self.bug, "tags", ["first-new-tag"])
    ...
    def test_attachment_removed(self):
        ...
        attachment_removed_activity = {
            ...
            "newvalue": None,
            "oldvalue": "%s %s" % (attachment_title, download_url),
        }
    ...
    def test_bugtask_added(self):
        ...
        task_added_notification = {
            ...
            "text": (
                ...
                "       Status: %s"
                % (
                    target.bugtargetname,
                    added_task.importance.title,
        ...
        task_added_notification = {
            ...
            "text": (
                ...
                % (
                    target.bugtargetname,
                    added_task.importance.title,
                    added_task.assignee.displayname,
                    added_task.assignee.name,
                    added_task.status.title,
                )
            ),
            "recipients": self.bug.getBugNotificationRecipients(
    ...
    def test_target_bugtask_to_product(self):
        ...
        # BugNotification get updated.
        with notify_modified(
            self.bug_task, ["target", "product"], user=self.user
        ) as bug_task_before_modification:
    ...
    def test_assign_bugtask(self):
        ...
        expected_notification = {
            "text": (
                ...
                % (self.bug_task.bugtargetname, self.user.unique_displayname)
            ),
            "person": self.user,
        }
    ...
    def _test_unassign_bugtask(self, bug_task, expected_recipients):
        ...
        expected_notification = {
            "text": (
                ...
                % (bug_task.bugtargetname, old_assignee.unique_displayname)
            ),
            "person": self.user,
            "recipients": expected_recipients,
        }
    ...
    def test_bugtask_deleted(self):
        ...
        task_deleted_notification = {
            "person": self.user,
            "text": ("** No longer affects: %s" % target.bugtargetname),
            "recipients": self.bug.getBugNotificationRecipients(
                level=BugNotificationLevel.LIFECYCLE
            ),
        }
    ...
    def test_marked_as_duplicate(self):
        ...
        master_notifications = (
            IStore(BugNotification)
            .find(BugNotification, bug=self.bug)
            .order_by(BugNotification.id)
        )
        new_notifications = [
            notification
            for notification in master_notifications
            if notification.id not in self.old_notification_ids
        ]
    ...
    def test_changed_duplicate(self):
        ...
        expected_notification = {
            ...
            "text": (
                ...
                "** This bug has been marked a duplicate of bug %d\n"
                "   %s"
                % (bug_one.id, bug_one.title, bug_two.id, bug_two.title)
            ),
    ...
    def test_changed_private_duplicate(self):
        ...
        expected_notification = {
            ...
            "text": (
                "** This bug is no longer a duplicate of private bug %d\n"
                "** This bug has been marked a duplicate of bug %d\n"
                "   %s" % (private_bug.id, public_bug.id, public_bug.title)
            ),
            "recipients": bug_recipients,
        }
    ...
    def test_convert_to_question_no_comment(self):
        ...
        status_notification = {
            "text": (
                "** Changed in: %s\n"
                "       Status: New => Invalid" % self.bug_task.bugtargetname
            ),
            "person": self.user,
            "recipients": self.bug.getBugNotificationRecipients(
                level=BugNotificationLevel.LIFECYCLE
    ...
    def test_description_changed_no_self_email(self):
        ...
        self.changeAttribute(self.bug, "description", "New description")

        # self.user is not included among the recipients.
        self.assertRecipients([self.product_metadata_subscriber])
    ...
    def test_bugtask_subscription_email_mentions_the_user(self):
        ...
        expected_message = "{} ({}) has assigned this bug to you".format(
