...
def _add_rate_limit_response(
    host, limit=5000, remaining=4000, reset=1000000000
):
    limits = {"limit": limit, "remaining": remaining, "reset": reset}
    responses.add(
        "GET",
        "https://%s/rate_limit" % host,
...
class TestGitHubUpdateBugWatches(TestCaseWithFactory):
    ...
    def test_process_one(self):
        ...
        responses.add(
            "GET",
            "https://api.github.com/repos/user/repository/issues/1234",
            json=remote_bug,
        )
        ...
        self.assertEqual(
            ...
            [
                (
                    watch.remotebug,
                    github.convertRemoteStatus(watch.remotestatus),
                )
                for watch in bug_tracker.watches
            ],
        )
