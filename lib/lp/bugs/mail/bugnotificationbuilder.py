...
from lp.app.enums import PRIVATE_INFORMATION_TYPES, SECURITY_INFORMATION_TYPES
...
def get_bugmail_from_address(person, bug):
    ...
    return format_address(
        displayname, "%s@%s" % (bug.id, config.launchpad.bugs_domain)
    )
...
class BugNotificationBuilder:
    ...
    def __init__(self, bug, event_creator=None):
        ...
        self.common_headers.append(
            ("X-Launchpad-Bug-Information-Type", bug.information_type.title)
        )
        ...
        if bug.information_type in PRIVATE_INFORMATION_TYPES:
            self.common_headers.append(("X-Launchpad-Bug-Private", "yes"))
        else:
            self.common_headers.append(("X-Launchpad-Bug-Private", "no"))
        ...
        if event_creator:
            self.common_headers.append(
                (
                    ...
                    "%s (%s)"
                    % (event_creator.displayname, event_creator.name),
                )
            )
