...
class AnnouncementDateWidget(SimpleInputWidget):
    ...
    def _getActionsVocabulary(self):
        action_names = [
            ...
            (
                "sometime",
                "At some time in the future when I come back to "
                "authorize it",
            ),
    ...
    def getInputValue(self):
        ...
        if action == "immediately" and announcement_date is not None:
            self._error = WidgetInputError(
                ...
                LaunchpadValidationError(
                    _(
                        "Please do not provide a date if you want to publish "
                        "immediately."
                    )
                ),
            )
