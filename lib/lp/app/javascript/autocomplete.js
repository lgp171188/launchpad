...
YUI.add('lp.app.autocomplete', function (Y) {
    ...
    namespace.getRepositoryCompletionURI = function (repo_node) {
        //var entered_uri = encodeURI(repo_node.get('value')).replace(":", "%3A");
        ...
        else if (entered_uri.startsWith("http:") || entered_uri.startsWith("https:")) {
            return null;
        }
        else if (entered_uri.startsWith("git@") || entered_uri.startsWith("git+")) {
            return null;
        }
        else {
