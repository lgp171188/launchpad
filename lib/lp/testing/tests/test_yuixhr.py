...
class TestYUITestFixtureController(TestCase):
    ...
    def test_render_javascript(self):
        ...
        self.assertEqual(
            "text/javascript", view.request.response.getHeader("Content-Type")
        )
        self.assertEqual(
            "no-cache", view.request.response.getHeader("Cache-Control")
        )
    ...
    def test_render_html(self):
        ...
        self.assertTrue(content.startswith("<!DOCTYPE html>"))
        self.assertEqual(
            "text/html", view.request.response.getHeader("Content-Type")
        )
        self.assertEqual(
            "no-cache", view.request.response.getHeader("Cache-Control")
        )
    ...
    def test_do_teardown(self):
        ...
        self.assertEqual("\n", "".join(result))
        self.assertEqual(
            "1", view.request.response.getHeader("Content-Length")
        )
    ...
    def test_do_teardown_resets_database_only_after_request_completes(self):
        ...
        self.assertRaises(DisconnectionError, products.getByName, name)
    ...
    def test_do_teardown_multiple(self):
        ...
        view()
        self.assertEqual("second", test_yuixhr_fixture._received[0][0])
        self.assertEqual("baseline", test_yuixhr_fixture._received[1][0])
        del test_yuixhr_fixture._received[:]
    ...
    def test_python_fixture_can_reload(self):
        ...
        with override_environ(INTERACTIVE_TESTS="1"):
            ...
            self.assertIsNone(
                getattr(
                    test_yuixhr_fixture._fixtures_["baseline"],
                    "scribble",
                    None,
                )
            )
    ...
    def test_index_page(self):
        ...
        self.assertThat(
            output,
            Contains('href="/+yuitest/lp/testing/tests/test_yuixhr_fixture'),
        )
