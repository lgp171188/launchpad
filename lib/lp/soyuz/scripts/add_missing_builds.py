...
class AddMissingBuilds(SoyuzScript):
    ...
    def add_missing_builds(
        self, archive, required_arches, distroseries, pocket
    ):
        ...
        if not distroseries_architectures:
            self.logger.error(
                "No architectures defined for %s, skipping" % distroseries.name
            )
            return

        architectures_available = set(distroseries.buildable_architectures)
        if not architectures_available:
            self.logger.error("Chroots missing for %s" % distroseries.name)
            return
        ...
        self.logger.info(
            "Creating builds in %s"
            % " ".join(
                arch_series.architecturetag for arch_series in doable_arch_set
            )
        )
