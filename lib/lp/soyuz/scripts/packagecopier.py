...
def update_files_privacy(pub_record):
    ...
    for obj, attr_name in package_files:
        ...
        if (
            lfa is None
            or lfa.restricted == archive.private
...
def check_copy_permissions(
    person, archive, series, pocket, sources, move=False
):
    ...
    for source in sources:
        ...
        override = policy.calculateSourceOverrides(
            {spn: SourceOverride(component=source.component)}
        )[spn]
...
class CopyChecker:
    ...
    def __init__(
        self, archive, include_binaries, strict_binaries=True, unembargo=False
    ):
    ...
    def _getInventoryKey(self, candidate):
        ...
        """
        return (
            candidate.source_package_name,
            candidate.source_package_version,
        )
    ...
    def _checkArchiveConflicts(self, source, series):
        ...
        if (
            destination_archive_conflicts.is_empty()
            and len(inventory_conflicts) == 0
        ):
        ...
        for candidate in destination_archive_conflicts:
            ...
            if (
                source.sourcepackagerelease.id
            ...
            if (
                candidate.distroseries.id == series.id
                and self.archive.id == source.archive.id
                and self.include_binaries
            ):
                continue
    ...
    def _checkConflictingFiles(self, source):
        ...
        source_files = [
            ...
            for sprf in source.sourcepackagerelease.files
        ]
