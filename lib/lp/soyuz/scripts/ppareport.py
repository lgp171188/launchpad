...
class PPAReportScript(LaunchpadScript):
    ...
    def ppas(self):
        ...
        results = store.using(*origin).find(Archive, *clauses)
    ...
    def checkOptions(self):
        ...
        if (
            self.options.gen_orphan_repos
            or self.options.gen_missing_repos
            or self.options.gen_over_quota
        ) and self.options.gen_user_emails:
            raise LaunchpadScriptFailure(
                "Users-list cannot be combined with other reports."
            )

        if (
            self.options.gen_orphan_repos or self.options.gen_missing_repos
