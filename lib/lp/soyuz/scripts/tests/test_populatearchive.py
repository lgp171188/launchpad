...
class TestPopulateArchiveScript(TestCaseWithFactory):
    ...
    def runWrapperScript(self, extra_args=None):
        ...
        if extra_args is None:
            extra_args = []
        script = os.path.join(config.root, "scripts", "populate-archive.py")
        args = [script, "-y"]
    ...
    def testNonvirtualized(self):
        ...
        copy_archive = self.runScript(
            archive_name="copy-archive-test",
            exists_before=False,
            exists_after=True,
            nonvirtualized=True,
            extra_args=["-a", "386"],
    ...
    def testMergeCopy(self):
        ...
        copy_archive = self.runScript(extra_args=extra_args, exists_after=True)
        self._verifyClonedSourcePackages(copy_archive, hoary)
        ...
        extra_args = ["--merge-copy", "--from-archive", second_stage.name]
    ...
    def testInvalidOriginArchiveName(self):
        ...
        This test should provoke a `SoyuzScriptError` exception.
        """
        extra_args = ["-a", "amd64", "--from-archive", "//"]
        self.runScript(
            extra_args=extra_args,
            exception_type=SoyuzScriptError,
    ...
    def testBuildsPendingAndSuspended(self):
        ...
        def build_in_wrong_state(build):
            ...
            return not (
                ...
                and build.buildqueue_record.status
                == BuildQueueStatus.SUSPENDED
            )
        ...
        builds = list(
            getUtility(IBinaryPackageBuildSet).getBuildsForArchive(archive)
        )
    ...
    def testDisabledDestinationArchive(self):
        ...
        disabled_archive = getUtility(IArchiveSet).new(
            ArchivePurpose.COPY,
            cprov,
            name="disabled-copy-archive",
            distribution=distro,
            description="disabled-copy-archive test",
            enabled=False,
        )

        extra_args = ["--from-user", "cprov", "--merge-copy"]
        self.runScript(
            copy_archive_name=disabled_archive.name,
            reason="",
            extra_args=extra_args,
            exception_type=SoyuzScriptError,
    ...
    def _prepareMergeCopy(self):
        ...
        test_publisher.getPubSource(
            sourcename="new-in-second-round",
            version="1.0",
            distroseries=hoary,
        ...
        sources = ubuntu.main_archive.getPublishedSources(
            distroseries=hoary, status=self.pending_statuses, name="alsa-utils"
        )
        for src in sources:
