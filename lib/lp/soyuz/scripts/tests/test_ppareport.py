...
class TestPPAReport(unittest.TestCase):
    ...
    def testDeniedOptionCombination(self):
        ...
        for kwargs in denied_combinations:
            ...
            self.assertRaises(LaunchpadScriptFailure, reporter.main)
    ...
    def testGetActivePPAs(self):
        ...
        self.assertEqual(
            [ppa.owner.name for ppa in reporter.ppas], ["cprov", "mark"]
        )

        reporter = self.getReporter(ppa_owner="cprov")
        self.assertEqual([ppa.owner.name for ppa in reporter.ppas], ["cprov"])

        reporter = self.getReporter(ppa_owner="foobar")
        self.assertEqual([ppa.owner.name for ppa in reporter.ppas], [])
    ...
    def testOverQuota(self):
        ...
        reporter = self.getReporter(quota_threshold=0.01)
        reporter.setOutput()
        reporter.reportOverQuota()
        self.assertEqual(
            reporter.output.getvalue().splitlines(),
            [
                ...
                "http://launchpad.test/~cprov/+archive/ubuntu/ppa | 1024 | 9",
                "http://launchpad.test/~mark/+archive/ubuntu/ppa | 1024 | 9",
                "",
            ],
        )
    ...
    def testRunMain(self):
        ...
        reporter = self.getReporter(
            gen_over_quota=True, gen_orphan_repos=True, gen_missing_repos=True
        )
        reporter.main()
        self.assertEqual(
            reporter.output.getvalue().splitlines(),
