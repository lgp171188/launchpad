...
def run_gina(options, ztm, target_section):
    ...
    try:
        arch_component_items = ArchiveComponentItems(
            package_root, pocket_distroseries, components, archs, source_only
        )
...
def attempt_source_package_import(
...
def import_sourcepackages(
    distro, packages_map, package_root, importer_handler
):
            ...
            try:
                try:
                    do_one_binarypackage(
                        distro, binary, archtag, package_root, importer_handler
                    )
                except psycopg2.Error:
                    log.exception(
