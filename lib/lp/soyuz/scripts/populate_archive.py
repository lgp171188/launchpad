...
class ArchivePopulator(SoyuzScript):
    ...
    def populateArchive(
        ...
        if from_user is not None:
            ...
            if origin_archive is not None:
                ...
                raise SoyuzScriptError("No PPA for user: '%s'" % from_user)
        ...
        pcr = getUtility(IPackageCopyRequestSet).new(
            ...
            the_destination,
            registrant,
            copy_binaries=include_binaries,
            reason=six.ensure_text(reason),
        )
    ...
    def _packageset_delta(self, origin, destination):
        ...
        """
        pkg_cloner = getUtility(IPackageCloner)
        pkg_cloner.packageSetDiff(origin, destination, self.logger)
    ...
    def mainTask(self):
        ...
        if not specified(opts.from_distribution):
            raise SoyuzScriptError("error: origin distribution not specified.")
        ...
        self.populateArchive(
            opts.from_archive,
            opts.from_distribution,
            opts.from_suite,
            opts.from_user,
            opts.component,
            opts.to_distribution,
            opts.to_suite,
            opts.to_archive,
            opts.to_user,
            opts.reason,
            opts.include_binaries,
            opts.arch_tags,
            opts.merge_copy_flag,
            opts.packageset_delta_flag,
            opts.packageset_tags,
            opts.nonvirtualized,
        )

    def add_my_options(self):
        """Parse CLI arguments for copy archive creation/population."""
