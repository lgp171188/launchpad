...
class TestDistroSeriesQueueRosettaTranslationsTarball(
    ...
    def setUp(self):
        ...
        self.source_changes_file = "%s_%s_source.changes" % (
            self.package_name,
            self.version,
        )
        self.bin_changes_file = "%s_%s_i386.changes" % (
            self.package_name,
            self.version,
        )
    ...
    def _getImportableFilesFromTarball(self):
        ...
        return [
            relpath(file_, "./source/")
            for file_ in tarball.getnames()
            if ".po" in file_
        ]
    ...
    def test_publish(self):
        ...
        with dbuser("poimport"):
            importer = TranslationsImport(
                "poimport", logger=self.logger, test_args=[]
            )
            importer.main()
    ...
    def uploadToPPA(self):
        ...
        self.assertEqual(
            bin_upload.queue_root.status, PackageUploadStatus.ACCEPTED
        )

        bin_upload.queue_root.realiseUpload()
        self.assertEqual(
            bin_upload.queue_root.status, PackageUploadStatus.DONE
        )
        self.assertEqual(
            bin_upload.queue_root.builds[0].build.status.name, "FULLYBUILT"
        )
        transaction.commit()
