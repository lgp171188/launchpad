...
from lp.soyuz.enums import BinaryPackageFileType, BinaryPackageFormat
...
class TestPublishingSet(BaseTestCaseWithThreeBuilds):
    ...
    def test_getUnpublishedBuildsForSources_one_published(self):
        ...
        self.factory.makeBinaryPackagePublishingHistory(
            ...
            archive=self.sources[0].archive,
...
class TestSourcePackagePublishingHistory(TestCaseWithFactory):
    ...
    def getURLsForSPPH(self, spph, include_meta=False):
        ...
        urls = [
            ProxiedSourceLibraryFileAlias(f.libraryfile, spph).http_url
            for f in spr.files
        ]
...
class TestBinaryPackagePublishingHistory(TestCaseWithFactory):
    ...
    def getURLsForBPPH(self, bpph, include_meta=False):
        ...
        urls = [
            ProxiedLibraryFileAlias(f.libraryfile, archive).http_url
            for f in bpr.files
        ]
    ...
    def makeBPPH(self, num_binaries=1):
        ...
        for count in range(num_binaries):
            self.factory.makeBinaryPackageFile(
                binarypackagerelease=bpr, filetype=filetypes[count % 2]
            )
    ...
    def test_source_package_version_private(self):
        ...
        self.assertRaises(
            Unauthorized, getattr, bpph, "source_package_version"
        )
