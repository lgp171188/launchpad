...
from lp.soyuz.enums import ArchivePurpose, PackageUploadCustomFormat
...
class TestNotificationRequiringLibrarian(TestCaseWithFactory):
    ...
    def test_calculate_subject_customfile(self):
        ...
        subject = calculate_subject(
            None, [], [customfile], archive, distroseries, pocket, "accepted"
        )
        self.assertEqual(expected_subject, subject)

    def _setup_notification(
        self, from_person=None, distroseries=None, spr=None
    ):
        if spr is None:
...
class TestNotification(TestCaseWithFactory):
    ...
    def test_calculate_subject_bprs(self):
        ...
        expected_subject = "[%s/%s] %s %s (Accepted)" % (
            archive.reference,
            distroseries.getSuite(pocket),
            bpr.build.source_package_release.name,
            bpr.version,
        )
        subject = calculate_subject(
    ...
    def test_getRecipientsForAction_good_emails(self):
        ...
        self.assertRecipientsEqual(
            [blamer, maintainer, changer], changes, blamer, maintainer, changer
        )
    ...
    def test_getRecipientsForAction_unsigned_copy_archive(self):
        ...
        self.assertRecipientsEqual(
            [], changes, None, maintainer, changer, purpose=ArchivePurpose.COPY
        )
                ...
                ContainsDict(
                    {
                        ...
                        "X-Launchpad-Notification-Type": Equals(
                            "package-upload"
                        ),
