...
class IDistroArchSeriesBinaryPackage(Interface):
    ...
    summary = Attribute(
        "A guessed summary for this package. Either "
    ...
    releases = Attribute(
        "All of the distroarchseries binary package "
        "releases that have been made for this package."
    )

    currentrelease = Attribute(
        """The latest published BinaryPackageRelease
    ...
    current_published = Attribute(
        "is last BinaryPackagePublishing "
    ...
    distro_source_package = Reference(
        IDistributionSourcePackage,
        title=_(
            "The DistributionSourcePackage that was used to generate the "
