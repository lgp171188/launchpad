...
from lp.testing import TestCase, login
...
class TestExtraBreadcrumbForLeafPageOnHierarchyView(BaseBreadcrumbTestCase):
    ...
    def test_non_default_page(self):
        ...
        self.assertEqual(
            [self.product_url, downloads_url], [crumb.url for crumb in crumbs]
        )
        self.assertEqual(
            "%s project files" % self.product.displayname, crumbs[-1].text
        )
    ...
    def test_facet_default_page(self):
        ...
        self.assertEqual(
            [self.product_url, bugs_url], [crumb.url for crumb in crumbs]
        )
