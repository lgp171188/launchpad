...
from testtools.matchers import Contains, ContainsDict, Equals, MatchesListwise
...
from lp.services.database.interfaces import IStore, IStoreSelector
...
from lp.services.webapp.interfaces import ILaunchpadApplication, ISession
from lp.services.webapp.login import OpenIDCallbackView, OpenIDLogin
from lp.services.webapp.servers import LaunchpadTestRequest
from lp.testing import TestCase, TestCaseWithFactory, logout
from lp.testing.browser import Browser, setUp
...
def SRegResponse_fromSuccessResponse_stubbed():
    def sregFromFakeSuccessResponse(cls, success_response, signed_only=True):
        return {
            "email": success_response.sreg_email,
...
def MacaroonResponse_fromSuccessResponse_stubbed():
    def macaroonFromFakeSuccessResponse(
        cls, success_response, signed_only=True
    ):
...
class TestOpenIDCallbackView(TestCaseWithFactory):
    ...
    def _createAndRenderView(
        self, response, view_class=StubbedOpenIDCallbackView, form=None
    ):
        if form is None:
    ...
    def test_never_used_account(self):
        ...
        openid_identifier = (
            ...
            .find(
                ...
                OpenIdIdentifier.account_id == person.account.id,
            )
            .order_by(OpenIdIdentifier.account_id)
            .order_by(OpenIdIdentifier.account_id)
            .first()
        )
        openid_response = FakeOpenIDResponse(
    ...
    def test_discharge_macaroon(self):
        ...
        openid_response = FakeOpenIDResponse(
            ...
            email=test_email,
            full_name="Foo User",
            discharge_macaroon_raw="dummy discharge",
        )
        form = {
            "starting_url": "http://launchpad.test/after-login",
...
def fill_login_form_and_submit(browser, email_address):
    assert (
        browser.getControl(name="field.email") is not None
...
class TestOpenIDReplayAttack(TestCaseWithFactory):
    ...
    def test_replay_attacks_do_not_succeed(self):
        ...
        self.assertEqual(
            "Nonce already used or out of range", extract_text(error_msg)
        )
...
class TestMissingServerShowsNiceErrorPage(TestCase):
    ...
    def test_missing_openid_server_shows_nice_error_page(self):
        ...
        browser = TestBrowser()
        self.assertRaises(
            HTTPError, browser.open, "http://launchpad.test/+login"
        )
        self.assertEqual(
            "503 Service Unavailable", browser.headers.get("status")
        )
        self.assertTrue(
...
class ForwardsCorrectly:
    ...
    def match(self, query_string):
        ...
        return Contains(expected_fragment).match(view.openid_request.return_to)
...
class TestOpenIDLogin(TestCaseWithFactory):
    ...
    def test_sreg_fields(self):
        ...
        self.assertEqual(
            ["email", "fullname"], sorted(sreg_extension.allRequestedFields())
        )
        self.assertEqual(
            sorted(sreg_extension.required),
    ...
    def test_macaroon_extension_added_with_macaroon_query(self):
        ...
        return_to_args = dict(
            parse_qsl(urlsplit(view.openid_request.return_to).query)
        )
        self.assertEqual(
...
class TestOpenIDRealm(TestCaseWithFactory):
    ...
    def test_realm_for_vhosts(self):
        ...
        # don't we can easily inspect what's in the form.
        self.assertEqual(
            "%s" % self.layer.appserver_root_url(ensureSlash=True),
            browser.getControl(name="openid.realm").value,
        )
...
def test_suite():
    ...
    suite.addTest(
        LayeredDocFileSuite("login.rst", setUp=setUp, layer=AppServerLayer)
    )
    return suite
