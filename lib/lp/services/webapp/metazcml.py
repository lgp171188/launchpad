...
from zope.browserpage.metaconfigure import page as original_page
from zope.browserpage.metaconfigure import pages as original_pages
...
def _isAuthorization(module, module_member):
    return (
        ...
        module.__name__ == module_member.__module__
        and IAuthorization.implementedBy(module_member)
    )
...
class ISecuredUtilityDirective(Interface):
    ...
    provides = GlobalObject(
        title="interface this utility provides", required=True
    )
...
class SecuredUtilityDirective:
    def __init__(
        self, _context, provides, class_=None, component=None, name=""
    ):
...
class IURLDirective(Interface):
    ...
    urldata = GlobalObject(
        title="Adapter to ICanonicalUrlData for this object.", required=False
    )
...
class IGlueDirective(Interface):
    ...
    module = GlobalObject(title="Module in which the classes are found.")
...
class IFaviconDirective(Interface):
    ...
    file = Path(title="Path to the image file", required=True)
...
def menus(_context, module, classes):
    ...
    for menuname in classes:
        ...
        for menutype in menutypes:
            ...
            raise TypeError(
                "class %r is not one of %s" % (menuclass, menutypes)
            )
...
class InterfaceInstanceDispatcher:
    """Dispatch getitem on names in the interface to the instance."""
...
class IPageDirective(
    zope.browserpage.metadirectives.IPageDirective, IAssociatedWithAFacet
):
...
class IPagesDirective(
    zope.browserpage.metadirectives.IPagesDirective, IAssociatedWithAFacet
):
...
def renamed_page(
    _context, for_, name, new_name, layer=IDefaultBrowserLayer, rootsite=None
):
...
def definePermission(
    _context, id, title, access_level="write", description=""
):
