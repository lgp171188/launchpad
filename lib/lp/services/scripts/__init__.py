...
def execute_zcml_for_scripts(
    use_web_security=False, scriptzcmlfilename=None, setup_interaction=True
):
    ...
    if "lp.testing.layers" in sys.modules:
        from lp.testing.layers import BaseLayer, FunctionalLayer, ZopelessLayer

        assert (
            not FunctionalLayer.isSetUp
        ), "Setting up Zopeless CA when Zopefull CA is already running"
        assert (
            not BaseLayer.isSetUp or ZopelessLayer.isSetUp
        ), """
...
def db_options(parser):
    ...
    parser.add_option(
        ...
        "--dbname",
        action="callback",
        callback=dbname_callback,
        type="string",
        dest="dbname",
        default=conn_string.dbname,
    ...
    parser.add_option(
        ...
        "--host",
        action="callback",
        callback=dbhost_callback,
        type="string",
        dest="dbhost",
        default=conn_string.host,
    ...
    parser.add_option(
        ...
        "--port",
        action="callback",
        callback=dbport_callback,
        type=int,
        dest="dbport",
        default=conn_string.port,
