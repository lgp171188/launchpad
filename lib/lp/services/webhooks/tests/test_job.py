...
from lp.testing.fixture import CaptureOops, ZopeUtilityFixture
...
class TestWebhookDeliveryJob(TestCaseWithFactory):
    ...
    def makeAndRunJob(
        ...
        hook = self.factory.makeWebhook(
            delivery_url="http://example.com/ep", secret=secret, active=active
        )
    ...
    def test_run_inactive(self):
        ...
        self.assertThat(
            ...
            MatchesStructure(
                ...
                json_data=ContainsDict(
                    {"result": MatchesDict({"webhook_deactivated": Is(True)})}
                ),
            ),
        )
        self.assertEqual([], reqs)
    ...
    def test_retry_delay(self):
        ...
        job.json_data["date_first_sent"] = (
            job.date_first_sent - timedelta(minutes=5)
        ).isoformat()
        self.assertEqual(timedelta(minutes=1), job.retry_delay)
        job.json_data["date_first_sent"] = (
            job.date_first_sent - timedelta(minutes=5)
        ...
        job.json_data["date_first_sent"] = (
            job.date_first_sent - timedelta(minutes=30)
        ).isoformat()
        self.assertEqual(timedelta(minutes=5), job.retry_delay)
        job.json_data["date_first_sent"] = (
            job.date_first_sent - timedelta(minutes=30)
    ...
    def test_retry_automatically_connection_error(self):
        ...
        job.json_data["date_first_sent"] = (
            job.date_first_sent - timedelta(hours=23)
        ).isoformat()
        self.assertTrue(job.retry_automatically)
        job.json_data["date_first_sent"] = (
            job.date_first_sent - timedelta(hours=24)
        ).isoformat()
    ...
    def test_retry_automatically_4xx(self):
        ...
        self.assertTrue(job.retry_automatically)
        job.json_data["date_first_sent"] = (
            job.date_first_sent - timedelta(minutes=59)
        ).isoformat()
        self.assertTrue(job.retry_automatically)
        job.json_data["date_first_sent"] = (
            job.date_first_sent - timedelta(hours=1)
        ).isoformat()
        self.assertFalse(job.retry_automatically)
    ...
    def assertRetriesWithLimitedEffort(self, url):
        ...
        job.json_data["date_first_sent"] = (
            job.date_first_sent - a_while_ago
        ).isoformat()
        self.assertFalse(job.retry_automatically)
