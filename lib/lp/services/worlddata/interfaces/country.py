...
__all__ = ["ICountry", "ICountrySet", "IContinent"]
...
from lp.services.fields import Description, Title
...
class ICountry(Interface):
    ...
    name = exported(
        TextLine(title=_("Country name"), required=True, constraint=valid_name)
    )
    ...
    languages = Attribute(
        "An iterator over languages that are spoken in " "that country."
    )
