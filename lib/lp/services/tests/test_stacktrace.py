...
class TestStacktrace(TestCase):
    ...
    def test_get_limit_helper(self):
        ...
        try:
            ...
            if (
                original_limit is self
                and getattr(sys, "tracebacklimit", self) is not self
            ):
                # Clean it off.
                del sys.tracebacklimit
            else:
    ...
    def test_get_frame_data_supplement_and_info(self):
        (
            ...
            get_frame(supplement={}, info="foo bar"), MY_LINE_NUMBER
        )
        self.assertEqual(
            dict(
                source_url=None,
    ...
    def test_extract_stack(self):
        ...
        filename, lineno, name, line, modname, supplement, info = extracted[-1]
        self.assertEqual(MY_FILE_NAME, filename)
    ...
    def test_extract_tb(self):
        ...
        filename, lineno, name, line, modname, supplement, info = extracted[0]
        self.assertEqual(MY_FILE_NAME, filename)
        self.assertIsInstance(lineno, int)
