...
class TestWorkItemsText(TestCase):
    ...
    def test_parseLine_without_status_fails(self):
        ...
        self.assertRaises(
            LaunchpadValidationError, self.field.parseLine, "Missing status"
        )

    def test_parseLine_without_title_fails(self):
        self.assertRaises(
            LaunchpadValidationError, self.field.parseLine, ":TODO"
        )
    ...
    def test_parse_orphaned_work_items(self):
        ...
        work_items_text = (
            "Work items for %s:\n%s: POSTPONED\n\nWork items for %s:\n%s: "
            "TODO\n\n%s: TODO"
            % (milestone_1, title_1, milestone_2, title_2, title_3)
        )
        parsed = self.field.parse(work_items_text)
        self.assertEqual(
            parsed,
    ...
    def test_only_workitems_get_sequence(self):
        ...
        self.assertEqual(
            [(wi["title"], wi["sequence"]) for wi in parsed],
            [
                ("A single work item", 0),
                ("A second work item", 1),
