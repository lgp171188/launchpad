...
from lp.services.mail.incoming import InactiveAccount, authenticateEmail
...
class TestDKIM(TestCaseWithFactory):
    ...
    def assertStronglyAuthenticated(self, principal, signed_message):
        if IWeaklyAuthenticatedPrincipal.providedBy(principal):
            self.fail(
                "expected strong authentication; got weak:\n"
                + self.get_dkim_log()
                + "\n\n"
    ...
    def assertWeaklyAuthenticated(self, principal, signed_message):
        if not IWeaklyAuthenticatedPrincipal.providedBy(principal):
            self.fail(
                "expected weak authentication; got strong:\n"
                + self.get_dkim_log()
                + "\n\n"
                + six.ensure_text(signed_message, errors="replace")
            )
    ...
    def makeMessageBytes(self, sender=None, from_address=None):
        ...
        buf = (
            b"From: "
            + six.ensure_binary(from_address)
            + b"\n"
            + b"""\
...
Why isn't this fixed yet?"""
    ...
    def test_dkim_broken_pubkey(self):
        ...
        self.assertEqual(
            principal.person.preferredemail.email, "foo.bar@canonical.com"
        )
        self.assertDkimLogContains("ERROR unknown algorithm in k= tag")
    ...
    def test_dkim_disabled(self):
        ...
        self.useFixture(
            FeatureFixture({"mail.dkim_authentication.disabled": "true"})
        )
        # A test that would normally pass will now fail
        self.assertRaises(self.failureException, self.test_dkim_valid_strict)

    def test_dkim_valid_strict(self):
        signed_message = self.fake_signing(
            self.makeMessageBytes(), canonicalize=(dkim.Simple, dkim.Simple)
        )
    ...
    def test_dkim_changed_from_address(self):
        ...
        # should come from From, not the dkim signature
        self.assertEqual(
            principal.person.preferredemail.email, "carlos@canonical.com"
        )
    ...
    def test_dkim_signed_by_other_address(self):
        ...
        self.factory.makeEmail(person=person, address="dkimtest@example.com")
        self.preload_dns_response()
        tweaked_message = self.makeMessageBytes(
    ...
    def test_dkim_signed_but_from_unknown_address(self):
        ...
        self.assertEqual(
            principal.person.preferredemail.email, "dkimtest@example.com"
        )
        self.assertWeaklyAuthenticated(principal, signed_message)
    ...
    def test_dkim_signed_but_from_unverified_address(self):
        ...
        person = self.factory.makePerson(
            email=from_address, name="dkimtest", displayname="DKIM Test"
        )
        ...
        self.assertEqual(principal.person.preferredemail.email, from_address)
        self.assertWeaklyAuthenticated(principal, signed_message)
        self.assertDkimLogContains(
    ...
    def test_dkim_signed_from_person_without_account(self):
        ...
        message_text = self.makeMessageBytes(
            sender=from_address, from_address=from_address
        )
