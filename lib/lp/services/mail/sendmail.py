...
def do_paranoid_envelope_to_validation(to_addrs):
    ...
    """
    assert (
        zisinstance(to_addrs, (list, tuple, set)) and len(to_addrs) > 0
    ), "Invalid To: %r" % (to_addrs,)
    for addr in to_addrs:
        assert zisinstance(addr, str) and bool(
            addr
...
def simple_sendmail(
    from_addr, to_addrs, subject, body, headers=None, bulk=True
):
    ...
    """
    ctrl = MailController(
        from_addr, to_addrs, subject, body, headers, bulk=bulk
    )
    return ctrl.send()
...
def validate_message(message):
    ...
    assert "subject" in message and bool(
        message["subject"]
    ), "No Subject: header"
...
def raw_sendmail(from_addr, to_addrs, raw_message, message_detail):
    ...
    """
    assert not isinstance(to_addrs, str), "to_addrs must be a sequence"
