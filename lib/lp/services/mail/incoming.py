...
from lp.services.webapp.interfaces import ILaunchBag, IPlacelessAuthUtility

# Match '\n' and '\r' line endings. That is, all '\r' that are not
# followed by a '\n', and all '\n' that are not preceded by a '\r'.
non_canonicalised_line_endings = re.compile(rb"((?<!\r)\n)|(\r(?!\n))")

# Match trailing whitespace.
trailing_whitespace = re.compile(rb"[ \t]*((?=\r\n)|$)")
...
def _verifyDkimOrigin(signed_message):
    ...
    if not _isDkimDomainTrusted(signing_domain):
        log.info(
            "valid DKIM signature from untrusted domain %s" % (signing_domain,)
        )
        return None
    for origin in ["From", "Sender"]:
        ...
        log.info(
            "DKIM signing domain %s doesn't match message origin; "
    ...
    try:
        for mail_id, raw_mail in mailbox.items():
            ...
            try:
                ...
                mailbox.delete(mail_id)
                _send_email_oops(
                    trans, log, mail, "Unhandled exception", file_alias_url
                )
    finally:
...
def handle_one_mail(
    log, mail, file_alias, file_alias_url, signature_timestamp_checker
):
    """Process one message.
    ...
    """
    log.debug(
        "processing mail from %r message-id %r"
