...
from lp.answers.model.questionjob import QuestionEmailJob, QuestionJob
...
class QuestionJobTestCase(TestCaseWithFactory):
    ...
    def test_instantiate(self):
        ...
        job = QuestionJob(question, QuestionJobType.EMAIL, metadata)
    ...
    def test_repr(self):
        question = self.factory.makeQuestion()
        metadata = []
        question_job = QuestionJob(question, QuestionJobType.EMAIL, metadata)
        self.assertEqual(
...
def make_question_job(
    ...
    return QuestionEmailJob.create(
        question, user, recipient_set, subject, body, headers
    )
...
class QuestionEmailJobTestCase(TestCaseWithFactory):
    ...
    def test_create(self):
        ...
        self.assertEqual(
            QuestionRecipientSet.SUBSCRIBER.name, job.metadata["recipient_set"]
        )
    ...
    def test_from_address(self):
        ...
        address = format_address(
            user.displayname, "question%s@answers.launchpad.net" % question.id
        )
    ...
    def test_recipients_subscriber(self):
        ...
        question_recipients = [
            person
            for _, person in job.question.getRecipients().getRecipientPersons()
            if person != job.question.owner
        ]
        self.assertContentEqual(question_recipients, recipients)
    ...
    def test_recipients_contact(self):
        ...
        job = make_question_job(self.factory, QuestionRecipientSet.CONTACT)
        recipients = [
            person for email, person in job.recipients.getRecipientPersons()
        ]
        self.assertContentEqual(
            job.question.target.getAnswerContactRecipients(None), recipients
        )
