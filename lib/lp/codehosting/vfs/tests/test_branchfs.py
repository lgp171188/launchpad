...
from breezy.tests import TestCase as BzrTestCase
...
from breezy.transport.memory import MemoryServer, MemoryTransport
from breezy.urlutils import escape, local_path_to_url
...
class TestLaunchpadServer(MixinBaseLaunchpadServerTests, BzrTestCase):
    ...
    def test_createBranch_error_translation(self):
        ...
        def check_exception(exception):
            ...
            self.assertEqual(
                ": Project 'no-such-product' does not exist.", exception.extra
            )
...
class LaunchpadInternalServerTests:
    ...
    def test_translate_control_dir_path(self):
        ...
        self.assertRaises(
            errors.NoSuchFile,
            transport.list_dir,
            "~%s/%s/.bzr/" % (branch.owner.name, branch.product.name),
        )
...
class TestLaunchpadInternalServer(
    MixinBaseLaunchpadServerTests, BzrTestCase, LaunchpadInternalServerTests
):
...
class TestDirectDatabaseLaunchpadServer(
    TestCaseWithFactory, LaunchpadInternalServerTests
):
    ...
    class VirtualServer(Server):
        ...
        def translateVirtualPath(self, virtual_path):
            return defer.succeed(
                (self._branch_transport, "prefix_" + virtual_path.lstrip("/"))
            )
...
class LaunchpadTransportTests:
    ...
    def assertFiresFailureWithSubstring(
        self, exc_type, msg, function, *args, **kw
    ):
    ...
    def test_readv_mapped_file(self):
        ...
        transport = self.getTransport()
        deferred = self._ensureDeferred(
            transport.readv, "%s/.bzr/hello.txt" % branch.unique_name, [(3, 2)]
        )
    ...
    def test_put_mapped_file(self):
        ...
        deferred = self._ensureDeferred(
            transport.put_bytes,
            "%s/.bzr/goodbye.txt" % branch.unique_name,
            b"Goodbye",
        )
    ...
    def test_rename(self):
        ...
        def check_file_was_renamed(dir_contents):
            ...
            deferred.addCallback(
                lambda ignored: self.assertEqual(
    ...
    def test_iter_files_recursive(self):
        ...
        deferred.addCallback(
            check_iter_result, list(backing_transport.iter_files_recursive())
        )
        return deferred
    ...
    def test_createBranch_invalid_package_name(self):
        ...
        return self.assertFiresFailureWithSubstring(
            errors.PermissionDenied,
            "is not a valid source package name",
            transport.mkdir,
            unique_name,
        )
    ...
    def test_rmdir(self):
        ...
        self.assertFiresFailure(
            errors.PermissionDenied, transport.rmdir, "~testuser/firefox/baz"
        )
...
class TestBranchChangedNotification(TestCaseWithTransport):
    ...
    def _replacement_branchChanged(
        self,
        user_id,
        branch_id,
        stacked_on_url,
    ...
    def get_server(self):
        if self._server is None:
            self._server = LaunchpadServer(
                XMLRPCWrapper(self.codehosting_api),
                self.requester.id,
    ...
    def _replacement_branchChanged(
        ...
        *format_strings,
        ...
        request = errorlog.ScriptRequest(
            [("source", branch_id), ("error-explanation", "An error occurred")]
        )
        try:
            raise TimeoutError()
        except TimeoutError:
...
class TestLaunchpadTransportReadOnly(BzrTestCase):
    ...
    def _setUpLaunchpadServer(
        self, user_id, codehosting_api, backing_transport
    ):
